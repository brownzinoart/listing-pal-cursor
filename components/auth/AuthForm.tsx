...
const AuthForm: React.FC<AuthFormProps> = ({ mode, onSubmit, loading, serverError }) => {
  ...
  const subtitle = mode === 'login' ? 'Login to access your dashboard.' : 'Sign up to get started.';
  ...
  return (
    <div className="min-h-[calc(100vh-12rem)] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8 bg-brand-panel p-8 sm:p-10 rounded-xl shadow-2xl border border-brand-border">
        <div>
          <h2 className="mt-6 text-center text-3xl font-extrabold text-brand-text-primary">{title}</h2>
          <p className="mt-2 text-center text-sm text-brand-text-secondary">{subtitle}</p>
        </div>
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div>
            <Button type="submit" variant="primary" fullWidth={true} isLoading={loading} size="lg">
              {buttonText}
            </Button>
          </div>
        </form>
        <div className="text-sm text-center">
          ...
          <Link to={switchLinkPath} className="font-medium text-brand-primary hover:text-brand-accent transition-colors">
