...
const RESOURCE_CARDS = [
  ...
  },
  {
    ...
    description:
      "Complete step-by-step real estate transaction workflow from listing to closing.",
  },
...
const DashboardPage: React.FC<DashboardPageProps> = ({ section }) => {
  ...
  const [selectedListingForContract, setSelectedListingForContract] = useState<
    string | null
  >(null);
  ...
  const getPageInfo = () => {
    const pageMap: { [key: string]: { title: string; subtitle: string } } = {
      dashboard: {
        title: "Good Morning, Pavano",
        subtitle: getMotivationalMessage(),
      },
      descriptions: {
        title: "Listing Descriptions",
        subtitle: "AI-powered property descriptions and content",
      },
      social: {
        title: "Social Media Posts",
        subtitle: "Create engaging social content for your listings",
      },
      videos: {
        title: "Video Studio",
        subtitle: "Create AI-powered property videos",
      },
      email: {
        title: "Email Marketing",
        subtitle: "Professional email templates and campaigns",
      },
      ads: {
        title: "Paid Advertising",
        subtitle: "Optimize your digital advertising campaigns",
      },
      interior: {
        title: "Interior Design",
        subtitle: "AI-powered room staging and design tools",
      },
      print: {
        title: "Print Materials",
        subtitle: "Professional flyers and marketing materials",
      },
      resources: {
        title: "Agent Resources",
        subtitle: "Tools and templates for real estate professionals",
      },
      settings: {
        title: "Settings",
    ...
    return (
      pageMap[section || "dashboard"] || {
        title: "Dashboard",
        subtitle: "Your real estate command center",
      }
    );
  };
  ...
  return (
    <ModernDashboardLayout title={title} subtitle={subtitle}>
      ...
      {section === "descriptions" && <DescriptionsDashboard />}
      {section === "email" && <EmailMarketingDashboard />}
      {section === "interior" && <InteriorDesignDashboard />}
      {section === "print" && <PrintMaterialsDashboard />}
      {section === "ads" && <PaidAdsDashboard />}
      {section === "social" && <SocialPostsDashboard />}
      {section === "videos" && <VideoStudioDashboard />}
      {section === "settings" && <SettingsDashboard />}
      {section &&
        section !== "dashboard" &&
        section !== "resources" &&
        section !== "ads" &&
            ...
            <div className="relative bg-white/5 backdrop-blur-lg border border-white/10 rounded-3xl p-12">
              <div className="text-center">
                ...
                <h2 className="text-3xl font-bold text-white mb-4 capitalize">
                  {NAV_LINKS.find((l) => l.path.endsWith(section))?.label ||
                    section}
                </h2>
                <p className="text-slate-300 text-lg mb-8">
                  This section will show actionable analytics and insights for{" "}
      ...
      {section === "resources" && (
        <div className="relative group">
                ...
                <div
                  ...
                  onClick={() => {
                    if (card.key === "contract-helper") {
                      console.log(
                        "Contract Helper clicked, listings:",
                        listings,
                      );
                      ...
                      if (listings.length === 0) {
                        console.log(
                          "No listings found, navigating to create new listing",
                        );
                        ...
                        if (listingId) {
                          console.log(
                            "Navigating to contract wizard for listing:",
                            listingId,
                          );
                          ...
                          alert(
                            "Error: Invalid listing data. Please try again.",
                          );
                        }
                      } else {
                        console.log(
                          "Multiple listings found, showing selection modal",
                        );
                      ...
                      console.log(
                        "Inspector Search clicked, listings:",
                        listings,
                      );
                      ...
                      if (listings.length === 0) {
                        ...
                        const listingId = listings[0].id;
                        console.log(
                          "Single listing found, navigating to inspector search",
                        );
                      ...
                      console.log(
                        "Contractor Search clicked, listings:",
                        listings,
                      );
                      ...
                      if (listings.length === 0) {
                        ...
                        const listingId = listings[0].id;
                        console.log(
                          "Single listing found, navigating to contractor search",
                        );
          ...
          <Dialog
            open={showContractHelper}
            onClose={() => setShowContractHelper(false)}
          ...
          >
            <div className="flex items-center justify-center min-h-screen px-4">
              ...
              <div className="relative bg-slate-800 border border-white/20 rounded-2xl shadow-2xl max-w-2xl w-full mx-auto p-8 z-10">
                <Dialog.Title className="text-2xl font-bold mb-6 text-white">
                  Create Contract
                </Dialog.Title>
                ...
                {listings.length === 0 ? (
                  <div className="text-center py-8">
                    <p className="text-slate-300 mb-4">
                      You need at least one listing to create a contract.
                    </p>
                    <Button
                      variant="gradient"
                      onClick={() => {
                  ...
                  <>
                    <p className="text-slate-300 mb-6">
                      Select a listing to create a contract for:
                    </p>
                    <div className="space-y-3 max-h-96 overflow-y-auto">
                      {listings.map((listing) => (
                        <div
                          key={listing.id}
                          onClick={() =>
                            setSelectedListingForContract(listing.id)
                          }
                          className={`
                          ...
                          <div className="flex items-start justify-between">
                            <div>
                              ...
                              <p className="text-slate-400 text-sm mt-1">
                                {listing.bedrooms} bed / {listing.bathrooms}{" "}
                                bath / {listing.squareFootage?.toLocaleString()}{" "}
                                sq ft
                              </p>
                      ...
                      <Button
                        ...
                        onClick={() => {
                          if (selectedListingForContract) {
                            navigate(
                              `/listings/${selectedListingForContract}/contract/basic`,
                            );
                            setShowContractHelper(false);
                            setSelectedListingForContract(null);
                          }
