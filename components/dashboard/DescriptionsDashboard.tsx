...
const mockDescriptionVersions = [
  {
    ...
    platform: "website",
    content:
      "Discover unparalleled luxury in this stunning modern masterpiece nestled in the heart of San Francisco...",
    wordCount: 247,
  ...
  },
  {
    ...
    platform: "mls",
    content:
      "Perfect family haven with spacious rooms, excellent schools nearby, and a beautiful backyard for kids to play...",
    wordCount: 189,
  ...
  },
  {
    ...
    platform: "social",
    content:
      "Prime investment property with excellent rental potential and strong appreciation history...",
    wordCount: 98,
  ...
  },
  {
    ...
    platform: "mls",
    content:
      "Charming starter home perfect for first-time buyers with modern updates and move-in ready condition...",
    wordCount: 156,
...
const DescriptionsDashboard: React.FC = () => {
  const [selectedView, setSelectedView] = useState<
    "all" | "by-listing" | "performance"
  >("all");
  const [selectedType, setSelectedType] = useState<
    "all" | "primary" | "alternative"
  >("all");
  const [selectedStatus, setSelectedStatus] = useState<
    "all" | "active" | "needs_review"
  >("all");
  ...
  const filteredDescriptions = useMemo(() => {
    return mockDescriptionVersions.filter((desc) => {
      ...
      if (selectedStatus !== "all" && desc.status !== selectedStatus)
        return false;
      return true;
    });
  ...
  const summaryMetrics = useMemo(() => {
    const totalViews = filteredDescriptions.reduce(
      (sum, desc) => sum + desc.performance.views,
      0,
    );
    const totalInquiries = filteredDescriptions.reduce(
      (sum, desc) => sum + desc.performance.inquiries,
      0,
    );
    const avgConversion =
      filteredDescriptions.reduce(
        (sum, desc) => sum + desc.performance.conversionRate,
        0,
      ) / filteredDescriptions.length;
    const avgEngagement =
      filteredDescriptions.reduce(
        (sum, desc) => sum + desc.performance.engagement,
        0,
      ) / filteredDescriptions.length;
  ...
  const descriptionsByListing = useMemo(() => {
    const grouped = filteredDescriptions.reduce(
      (acc, desc) => {
    ...
    <div className="space-y-8">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-8">
            <div>
              ...
              <p className="text-slate-400">
                Manage, optimize, and track your property descriptions
              </p>
            </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-6">
            <div className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-2xl font-bold text-white">
                  {summaryMetrics.totalDescriptions}
                </span>
              </div>
            ...
            <div className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-2xl font-bold text-white">
                  {summaryMetrics.avgConversion.toFixed(1)}%
                </span>
              </div>
            ...
            <div className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-2xl font-bold text-white">
                  {summaryMetrics.avgEngagement.toFixed(0)}%
                </span>
              </div>
                  ...
                  <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center space-x-3 mb-3">
                        ...
                        <span
                          className={`px-2 py-1 rounded-lg text-xs font-medium border ${getVersionBadgeColor(description.version)}`}
                        >
                          {description.version}
                        </span>
                      ...
                      <div className="flex items-center space-x-6 text-sm">
                        ...
                        <span className="text-slate-400">
                          {description.performance.views} views
                        </span>
                        <span className="text-emerald-400">
                          {description.performance.inquiries} inquiries
                        </span>
                        <span className="text-blue-400">
                          {description.performance.conversionRate}% conversion
                        </span>
                      </div>
      ...
      {selectedView === "by-listing" && (
        <div className="space-y-6">
          {Object.entries(descriptionsByListing).map(
                      ...
                      >
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center space-x-2">
                            {getStatusIcon(description.status)}
                            <span
                              className={`px-2 py-1 rounded text-xs font-medium border ${getVersionBadgeColor(description.version)}`}
                            >
                              {description.version}
                            </span>
                            {description.type === "primary" && (
                        ...
                        <div className="grid grid-cols-2 gap-4 text-xs">
                          ...
                          <div>
                            ...
                            <span className="text-blue-400 ml-1">
                              {description.performance.conversionRate}%
                            </span>
                          </div>
                          <div>
        ...
        <div className="relative group">
          ...
          <div className="relative bg-white/5 backdrop-blur-lg border border-white/10 rounded-3xl p-8">
            ...
            <div className="space-y-4">
              {[...filteredDescriptions]
                .sort(
                  (a, b) => b.performance.leadScore - a.performance.leadScore,
                )
                    ...
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="text-right">
                        <div className="text-2xl font-bold text-white">
                          {description.performance.leadScore}
                        </div>
