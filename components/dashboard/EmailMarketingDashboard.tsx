...
const EMAIL_THEMES: { id: EmailTheme; name: string; description: string; icon: string; color: string }[] = [
  { id: 'NEW_LISTING', name: 'New Listing Alert', description: 'Announce a fresh property on the market', icon: '🏡', color: 'emerald' },
  { id: 'OPEN_HOUSE', name: 'Open House Invitation', description: 'Invite prospects to view the property', icon: '🏠', color: 'blue' },
  { id: 'PRICE_REDUCTION', name: 'Price Reduction Notice', description: 'Alert about updated pricing', icon: '💰', color: 'amber' },
  { id: 'UNDER_CONTRACT', name: 'Under Contract Update', description: 'Notify that property is pending sale', icon: '📝', color: 'purple' },
  { id: 'EXCLUSIVE_SHOWING', name: 'Exclusive Showing', description: 'Private viewing invitation for VIP clients', icon: '⭐', color: 'pink' },
  { id: 'MARKET_UPDATE', name: 'Market Report', description: 'Share neighborhood market insights', icon: '📊', color: 'indigo' },
  { id: 'FOLLOW_UP', name: 'Follow-up Check-in', description: 'Professional follow-up with prospects', icon: '🤝', color: 'teal' },
  { id: 'COMING_SOON', name: 'Coming Soon Teaser', description: 'Build anticipation for upcoming listing', icon: '🔜', color: 'rose' },
];
...
const EmailMarketingDashboard: React.FC = () => {
  ...
  const filteredContent = useMemo(() => {
    return mockEmailContent.filter(content => {
      if (selectedTheme !== 'all' && content.theme !== selectedTheme) return false;
      if (searchTerm && !content.subject.toLowerCase().includes(searchTerm.toLowerCase()) && 
          !content.listingAddress.toLowerCase().includes(searchTerm.toLowerCase())) return false;
      return true;
    });
  ...
  const summaryMetrics = useMemo(() => {
    ...
    const avgWordCount = mockEmailContent.reduce((sum, c) => sum + c.wordCount, 0) / mockEmailContent.length;
    ...
    <div className="space-y-8">
      ...
      <div className="relative group">
        ...
        <div className="relative bg-white/5 backdrop-blur-lg border border-white/10 rounded-3xl p-8">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-8">
            <div>
              <h2 className="text-3xl font-bold text-white mb-2">Email Content Center</h2>
              <p className="text-slate-400">Organize and generate email content for your listings</p>
            </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-2xl font-bold text-white">{summaryMetrics.totalContent}</span>
              </div>
            ...
            <div className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-2xl font-bold text-white">{summaryMetrics.favoriteCount}</span>
              </div>
            ...
            <div className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-2xl font-bold text-white">{summaryMetrics.avgWordCount}</span>
              </div>
            ...
            <div className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-2xl font-bold text-white">{summaryMetrics.themeVariety}</span>
              </div>
      ...
      {viewMode === 'generate' && (
        <div className="relative group">
          ...
          <div className="relative bg-white/5 backdrop-blur-lg border border-white/10 rounded-3xl p-8">
            <h3 className="text-2xl font-bold text-white mb-6">Quick Generate Email</h3>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="space-y-6">
                ...
                <div>
                  <label className="block text-sm font-medium text-white mb-3">Email Theme</label>
              ...
              <div className="space-y-6">
                <div className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-6">
                  ...
                  {selectedListing && selectedGenerateTheme ? (
                    <div className="space-y-4">
                      ...
                      <div className="bg-white/5 rounded-lg p-4">
                        <h5 className="font-medium text-white mb-2">Email Theme</h5>
                        <div className="flex items-center space-x-2">
                          <span className="text-lg">{EMAIL_THEMES.find(t => t.id === selectedGenerateTheme)?.icon}</span>
                          <span className="text-slate-300 text-sm">
                            {EMAIL_THEMES.find(t => t.id === selectedGenerateTheme)?.name}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="space-y-3">
                        ...
                        <p className="text-slate-400 text-xs text-center">
                          This will open the email generator with your selected property and theme
                        </p>
                      </div>
                    </div>
                  ) : (
                    <div className="text-center py-6">
                      ...
                      <p className="text-slate-300 text-sm mb-2">Ready to generate</p>
                      <p className="text-slate-400 text-xs">
                ...
                <div className="bg-blue-500/10 border border-blue-500/20 rounded-2xl p-6">
                  ...
                  <p className="text-slate-300 text-sm">
                    Generated emails integrate with your property data to create personalized, compelling content. 
                    You can edit and customize the content before saving to your library.
                  </p>
                </div>
              </div>
            </div>
