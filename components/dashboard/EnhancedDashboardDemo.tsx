...
import TimePeriodSelector, { useTimePeriod, TimePeriod } from '../shared/TimePeriodSelector';
    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <DashboardSection>
          <BarChart
            ...
            xAxisKey="month"
            bars={[
              { key: 'leads', color: '#805AD5', name: 'Leads' }
            ]}
          />
        </DashboardSection>
      </div>
        ...
        <DashboardSection>
          <AreaChart
            ...
            areas={[
              ...
              { key: 'inquiries', color: '#4A55C7', name: 'Inquiries', fillOpacity: 0.2 }
            ]}
          />
        </DashboardSection>
        ...
        <DashboardSection title="Performance Indicators" padding="md">
          <div className="space-y-6">
            ...
            <div className="text-center">
              <h4 className="text-sm font-medium text-brand-text-secondary mb-3">Satisfaction Rate</h4>
              <ProgressRing 
                progress={87} 
                size="lg" 
                color="success"
            ...
            {/* Multi Progress Ring */}
            <div className="text-center">
              <h4 className="text-sm font-medium text-brand-text-secondary mb-3">Traffic Sources</h4>
              <MultiProgressRing 
                ...
                {trafficChannels.map((channel, index) => (
                  <div key={index} className="flex items-center justify-between text-xs">
                    <div className="flex items-center gap-2">
                      ...
                      />
                      <span className="text-brand-text-secondary">{channel.label}</span>
                    </div>
                    <span className="text-brand-text-primary font-medium">{channel.value}%</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      >
        <div className="space-y-4">
          {[
            { icon: <HomeIcon className="h-5 w-5" />, title: "New listing added", time: "2 hours ago", color: "text-blue-500" },
            { icon: <UserGroupIcon className="h-5 w-5" />, title: "Client meeting scheduled", time: "4 hours ago", color: "text-green-500" },
            { icon: <ChartBarIcon className="h-5 w-5" />, title: "Monthly report generated", time: "1 day ago", color: "text-purple-500" },
          ].map((item, index) => (
            <div key={index} className="flex items-center gap-4 p-3 bg-brand-background rounded-lg">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-1 mt-1">
                  ...
                  <span className="text-xs text-brand-text-tertiary">{item.time}</span>
                </div>
              </div>
            </div>
          ))}
