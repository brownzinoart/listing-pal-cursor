...
const InteriorDesignDashboard: React.FC = () => {
  ...
  const [selectedDesignStyle, setSelectedDesignStyle] = useState<AiDesignStyleId | 'all'>('all');
  ...
  const filteredContent = useMemo(() => {
    return mockRoomRedesigns.filter(redesign => {
      if (selectedRoomType !== 'all' && redesign.roomType !== selectedRoomType) return false;
      if (selectedDesignStyle !== 'all' && redesign.designStyle !== selectedDesignStyle) return false;
      if (searchTerm && !redesign.title.toLowerCase().includes(searchTerm.toLowerCase()) && 
          !redesign.listingAddress.toLowerCase().includes(searchTerm.toLowerCase())) return false;
      return true;
    });
  ...
  const summaryMetrics = useMemo(() => {
    ...
    const propertiesEnhanced = new Set(mockRoomRedesigns.map(r => r.listingId)).size;
    ...
    const totalClientDownloads = mockRoomRedesigns.reduce((sum, r) => {
      ...
      const downloadsByStyle = {
        'modern': 45, 'scandinavian': 38, 'industrial': 29, 'contemporary': 35,
        'minimalist': 31, 'bohemian': 22, 'traditional': 28, 'midcenturymodern': 33,
        'glamorous': 19, 'rustic': 25
      };
    ...
    const stylePerformance = mockRoomRedesigns.reduce((acc, r) => {
      if (!acc[r.designStyle]) acc[r.designStyle] = 0;
      acc[r.designStyle]++;
      return acc;
    }, {} as Record<string, number>);
    
    const topStyle = Object.entries(stylePerformance)
  ...
  const getRoomTypeInfo = (roomType: string) => {
    return ROOM_TYPES.find(t => t.id === roomType) || { name: roomType, icon: '🏠' };
  };

  const getDesignStyleInfo = (styleId: AiDesignStyleId) => {
    return AI_DESIGN_STYLES.find(s => s.id === styleId) || { name: styleId, description: '' };
  };
    ...
    <div className="space-y-8">
      ...
      <div className="relative group">
        ...
        <div className="relative bg-white/5 backdrop-blur-lg border border-white/10 rounded-3xl p-8">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-8">
            <div>
              <h2 className="text-3xl font-bold text-white mb-2">Interior Design Center</h2>
              <p className="text-slate-400">Boost property appeal and inquiries with AI room transformations</p>
            </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-2xl font-bold text-white">{summaryMetrics.propertiesEnhanced}</span>
              </div>
            ...
            <div className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-2">
                <span className="text-2xl">📥</span>
                <span className="text-2xl font-bold text-white">{summaryMetrics.totalClientDownloads}</span>
              </div>
            ...
            <div className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-2">
                <span className="text-2xl">📈</span>
                <span className="text-2xl font-bold text-white">+{summaryMetrics.avgInquiryLift}%</span>
              </div>
            ...
            <div className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-2">
                <span className="text-2xl">🎯</span>
                <span className="text-lg font-bold text-white capitalize">{getDesignStyleInfo(summaryMetrics.topPerformingStyle as AiDesignStyleId).name}</span>
              </div>
            ...
            {viewMode === 'content' && (
              <div className="flex items-center space-x-4">
                ...
                <select
                  ...
                  onChange={(e) => setSelectedDesignStyle(e.target.value as any)}
                  className="bg-white/10 border border-white/20 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:ring-2 focus:ring-purple-500"
                >
      ...
      {viewMode === 'generate' && (
        <div className="relative group">
          <div className="absolute inset-0 bg-gradient-to-r from-purple-500/10 to-pink-500/10 rounded-3xl blur-xl"></div>
          <div className="relative bg-white/5 backdrop-blur-lg border border-white/10 rounded-3xl p-8">
            <h3 className="text-2xl font-bold text-white mb-6">Quick Generate Room Redesign</h3>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="space-y-6">
                ...
                <div>
                  <label className="block text-sm font-medium text-white mb-3">Design Style</label>
              ...
              <div className="space-y-6">
                <div className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-6">
                  ...
                  {selectedListing ? (
                    <div className="space-y-4">
                      ...
                      <div className="bg-white/5 rounded-lg p-4">
                        <h5 className="font-medium text-white mb-2">Room & Style</h5>
                        <div className="flex items-center space-x-3 mb-2">
                          <span className="text-lg">{getRoomTypeInfo(selectedGenerateRoomType).icon}</span>
                        ...
                        <div className="text-slate-400 text-xs">
                          {getDesignStyleInfo(selectedGenerateStyle).description}
                        </div>
                      </div>
                      ...
                      <div className="space-y-3">
                        ...
                        <p className="text-slate-400 text-xs text-center">
                          This will open the AI room redesign tool with your selected options
                        </p>
                      </div>
                    </div>
                  ) : (
                    <div className="text-center py-6">
                      ...
                      <p className="text-slate-300 text-sm mb-2">Ready to redesign</p>
                      <p className="text-slate-400 text-xs">
                ...
                <div className="bg-purple-500/10 border border-purple-500/20 rounded-2xl p-6">
                  <h4 className="text-lg font-semibold text-purple-400 mb-2">🎨 How It Works</h4>
                  <div className="space-y-2 text-slate-300 text-sm">
                    <p>1. Upload or select a room photo from your listing</p>
                    <p>2. Choose the room type for accurate AI classification</p>
                    <p>3. Pick your preferred design style</p>
                    <p>4. AI generates a professional redesign in 30-60 seconds</p>
                  </div>
                </div>
