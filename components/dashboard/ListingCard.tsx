...
import { BuildingOfficeIcon, PencilIcon, TrashIcon, MapPinIcon } from '@heroicons/react/24/outline';
...
const ListingCard: React.FC<ListingCardProps> = ({ listing, onDelete }) => {
  ...
  const cityState = listing.address.split(',').slice(1,3).join(',').trim() || 'N/A';
    ...
    >
      ...
      <div className="p-5 flex flex-col flex-grow">
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm text-brand-text-tertiary mb-6 border-t border-brand-border pt-4 mt-auto">
          ...
          <div className="flex justify-between">
            <span>Sq Ft:</span>
            <span className="font-medium">{listing.squareFootage.toLocaleString()}</span>
          </div>
          <div className="flex justify-between">
            <span>Built:</span>
