...
const ModernPortfolioAnalytics: React.FC = () => {
  ...
  const heroMetrics = useMemo(() => {
    ...
    const hotProperties = propertyPerformanceData.filter(
      (p) => p.performanceTemp === "hot",
    ).length;
    const avgDaysOnMarket = Math.round(
      propertyPerformanceData.reduce((sum, p) => sum + p.daysOnMarket, 0) /
  ...
  const needsAttention = useMemo(() => {
    return propertyPerformanceData.filter((p) => p.needsAttention).slice(0, 3);
  }, []);
      ...
      >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-sm">
              <div>
                ...
                <p className="text-slate-300">
                  Potential buyers who contacted you about properties via
                  website forms, phone calls, open houses, or referrals.
                </p>
              </div>
              <div>
                <h4 className="text-blue-400 font-medium mb-2">Showings</h4>
                <p className="text-slate-300">
                  In-person or virtual property tours scheduled with qualified
                  prospects who expressed serious interest.
                </p>
              </div>
              <div>
                <h4 className="text-amber-400 font-medium mb-2">Offers</h4>
                <p className="text-slate-300">
                  Formal purchase offers submitted by buyers, representing
                  concrete interest in your listings.
                </p>
              </div>
            </div>
            <div className="mt-4 pt-4 border-t border-white/10">
              <p className="text-slate-400 text-xs">
                Data sources: CRM systems, website analytics, manual tracking,
            ...
            <div className="relative bg-white/10 backdrop-blur-lg border border-white/20 rounded-3xl p-8 hover:bg-white/15 transition-all duration-300">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="text-right">
                  ...
                  <p className="text-3xl font-bold text-white">
                    {heroMetrics.monthlyLeads}
                  </p>
                </div>
              </div>
              <div
                className={`flex items-center text-sm ${heroMetrics.leadGrowth >= 0 ? "text-emerald-400" : "text-red-400"}`}
              >
                <ArrowTrendingUpIcon className="h-4 w-4 mr-1" />
                {heroMetrics.leadGrowth >= 0 ? "+" : ""}
                {heroMetrics.leadGrowth}% vs last month
              </div>
            </div>
          </div>
          ...
          <div className="relative bg-white/5 backdrop-blur-lg border border-white/10 rounded-3xl p-8 overflow-hidden">
            <div className="mb-6">
              ...
              <p className="text-slate-400">
                Perfect for showing clients your growing pipeline and market
                expertise
              </p>
            </div>
              ...
              <div className="space-y-4">
                {topProperties.map((property) => (
                  <div
                    ...
                    className="bg-white/5 rounded-2xl p-4 hover:bg-white/10 transition-all duration-200"
                  >
                    ...
                    <div className="mt-3 flex items-center">
                      <div className="flex-1 bg-slate-700 rounded-full h-2">
                        <div
                          ...
                          style={{
                            width: `${Math.min(100, (property.monthlyLeads / 10) * 100)}%`,
                          }}
                        ></div>
                      </div>
            ...
            <div className="relative bg-white/5 backdrop-blur-lg border border-white/10 rounded-3xl p-8">
              <div className="flex items-center justify-between mb-6">
                ...
                <span className="px-3 py-1 bg-amber-500/20 text-amber-400 rounded-full text-sm font-medium">
                  ...
                  {needsAttention.length === 1 ? "property" : "properties"}
                </span>
              </div>
              {needsAttention.length > 0 ? (
                <div className="space-y-4">
                  {needsAttention.map((property) => (
                    <div
                      key={property.listingId}
                      className="bg-white/5 rounded-2xl p-4 hover:bg-white/10 transition-all duration-200 border-l-4 border-amber-400"
                    >
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-red-400 text-sm font-medium">
                            {property.monthlyLeads} leads/month
                          </p>
                        </div>
                      </div>
                      <div className="bg-amber-500/10 rounded-lg p-3">
                        ...
                        <p className="text-amber-300 text-sm">
                          {property.actionNeeded ||
                            "Review pricing and marketing strategy"}
                        </p>
                      </div>
                      <div className="mt-3 flex items-center justify-between">
                        <span className="text-slate-400 text-xs">
                          Potential monthly commission loss: $
                          {((property.price * 0.03) / 12).toLocaleString()}
                        </span>
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-slate-400 text-sm mt-1">
                    All properties are performing within target metrics
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="relative bg-white/5 backdrop-blur-lg border border-white/10 rounded-3xl p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-gradient-to-br from-blue-500/10 to-blue-600/5 rounded-2xl p-6 border border-blue-500/20">
                ...
                <p className="text-slate-300 text-sm mb-3">
                  "My portfolio consistently outperforms the local market, with
                  properties selling 23% faster than average."
                </p>
              ...
              <div className="bg-gradient-to-br from-emerald-500/10 to-emerald-600/5 rounded-2xl p-6 border border-emerald-500/20">
                ...
                <p className="text-slate-300 text-sm mb-3">
                  "My marketing attracts serious buyers - 85% of inquiries
                  convert to showings or offers."
                </p>
              ...
              <div className="bg-gradient-to-br from-amber-500/10 to-amber-600/5 rounded-2xl p-6 border border-amber-500/20">
                <div className="flex items-center mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-amber-400 text-sm font-medium">
                      ${(heroMetrics.totalValue / 1000000).toFixed(1)}M under
                      management
                    </p>
                  </div>
                </div>
                <p className="text-slate-300 text-sm mb-3">
                  "I currently manage $
                  {(heroMetrics.totalValue / 1000000).toFixed(1)} million in
                  active listings, demonstrating trusted expertise in your
                  market."
                </p>
            ...
            <div className="mt-8 bg-gradient-to-r from-purple-500/10 to-pink-500/10 rounded-2xl p-6 border border-purple-500/20">
              ...
              <p className="text-slate-300 text-sm mb-4">
                Based on current lead velocity and market conditions, your
                portfolio could support 2-3 additional premium listings. Spring
                market approaching - perfect timing for expansion.
              </p>
              <div className="flex items-center justify-between">
                <span className="text-purple-400 text-xs font-medium">
                  Projected additional monthly commission: $
                  {((heroMetrics.totalValue * 0.02) / 12).toLocaleString()}
                </span>
        ...
        <div className="text-center pb-12">
          <h3 className="text-white text-lg font-semibold mb-6">
            Ready to impress clients and grow your business?
          </h3>
          <div className="flex flex-wrap justify-center gap-4">
            <button className="px-8 py-4 bg-gradient-to-r from-emerald-500 to-teal-500 text-white font-semibold rounded-2xl hover:from-emerald-600 hover:to-teal-600 transition-all duration-200 transform hover:scale-105 shadow-lg">
              ...
              <div className="text-emerald-100 text-xs font-normal mt-1">
            ...
            <button className="px-8 py-4 bg-gradient-to-r from-blue-500 to-purple-500 text-white font-semibold rounded-2xl hover:from-blue-600 hover:to-purple-600 transition-all duration-200 transform hover:scale-105 shadow-lg">
              ...
              <div className="text-blue-100 text-xs font-normal mt-1">
            ...
            <button className="px-8 py-4 bg-white/10 backdrop-blur-lg border border-white/20 text-white font-semibold rounded-2xl hover:bg-white/15 transition-all duration-200 shadow-lg">
              Share Success Story
              <div className="text-slate-300 text-xs font-normal mt-1">
                Social media ready
              </div>
            </button>
          </div>
          ...
          <div className="mt-8 text-center">
            <p className="text-slate-400 text-sm">
              <span className="text-slate-300 font-medium">Pro Tip:</span> Show
              this dashboard to prospects during listing presentations to
              demonstrate your tech-savvy approach and portfolio success.
            </p>
          </div>
        </div>
      </ModernDashboardLayout>
