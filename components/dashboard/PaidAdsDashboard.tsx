...
const PaidAdsDashboard: React.FC = () => {
  ...
  const filteredCampaigns = useMemo(() => {
    return campaigns.filter(campaign => {
      const matchesSearch = campaign.listingAddress.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  }, [campaigns, searchTerm, statusFilter, platformFilter, sortField, sortDirection]);
  ...
  const overallMetrics = useMemo(() => {
    ...
    return {
      ...
      avgCTR: activeCampaigns.length > 0 
        ? activeCampaigns.reduce((sum, c) => sum + c.metrics.ctr, 0) / activeCampaigns.length 
        : 0,
      totalConversions: activeCampaigns.reduce((sum, c) => sum + c.metrics.conversions, 0),
      totalImpressions: activeCampaigns.reduce((sum, c) => sum + c.metrics.impressions, 0),
    };
  }, [campaigns]);
