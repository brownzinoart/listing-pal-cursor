...
import { LineChart, BarChart, PieChart, AreaChart, ScatterChart } from '../shared/charts';
...
const getPropertyTypeData = (selectedListings: string[]) => {
  const selectedProperties = mockListings.filter(l => selectedListings.includes(l.id));
  const typeCount = selectedProperties.reduce((acc, property) => {
...
const getPriceAnalysisData = (selectedListings: string[]) => {
  ...
  return ranges.map(range => {
    const propertiesInRange = selectedProperties.filter(p => p.price >= range.min && p.price < range.max);
    const avgDays = propertiesInRange.length > 0 
      ? Math.round(propertiesInRange.reduce((sum, p) => {
          const performance = propertyPerformanceData.find(perf => perf.listingId === p.id);
    ...
    return {
      ...
      avgPrice: propertiesInRange.length > 0 
        ? Math.round(propertiesInRange.reduce((sum, p) => sum + p.price, 0) / propertiesInRange.length)
        : 0
    };
...
const PortfolioAnalyticsDashboard: React.FC = () => {
  ...
  const propertyTypeData = useMemo(() => getPropertyTypeData(finalSelectedListings), [finalSelectedListings]);
  const priceAnalysisData = useMemo(() => getPriceAnalysisData(finalSelectedListings), [finalSelectedListings]);
  
  // Filter performance data based on selected listings
  const filteredPerformanceData = useMemo(() => {
    return propertyPerformanceData.filter(perf => finalSelectedListings.includes(perf.listingId));
  }, [finalSelectedListings]);

  // Market segment data for selected properties
  const selectedMarketSegments = useMemo(() => {
    const selectedProperties = mockListings.filter(l => finalSelectedListings.includes(l.id));
    const luxury = selectedProperties.filter(l => l.price >= 2000000);
    const midMarket = selectedProperties.filter(l => l.price >= 500000 && l.price < 2000000);
    const entryLevel = selectedProperties.filter(l => l.price < 500000);

    return [
      { name: 'Luxury ($2M+)', value: luxury.length, color: '#4A55C7', percentage: Math.round((luxury.length / selectedProperties.length) * 100) },
      { name: 'Mid-Market', value: midMarket.length, color: '#38A169', percentage: Math.round((midMarket.length / selectedProperties.length) * 100) },
      { name: 'Entry-Level', value: entryLevel.length, color: '#805AD5', percentage: Math.round((entryLevel.length / selectedProperties.length) * 100) }
  ...
  const currentInsights = useMemo(() => {
    ...
    const topPerformer = filteredPerformanceData.reduce((max, current) => 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        <DashboardSection>
          <BarChart
            ...
            formatTooltip={(value, name) => {
              if (name === 'ROI') return typeof value === 'string' ? value : `${value}x`;
          ...
          <div className="space-y-4">
            <div className="p-4 bg-brand-background rounded-lg border border-green-500/20">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="font-medium text-brand-text-primary">Top Performer</span>
              </div>
              <p className="text-sm text-brand-text-secondary">
                456 Oak Avenue is generating 52 leads with highest engagement rate at 4.2%
              </p>
            </div>
            ...
            <div className="p-4 bg-brand-background rounded-lg border border-blue-500/20">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="font-medium text-brand-text-primary">Market Timing</span>
              </div>
              <p className="text-sm text-brand-text-secondary">
                Properties in $2M+ range are selling 25% faster than average market time
              </p>
            </div>
            ...
            <div className="p-4 bg-brand-background rounded-lg border border-yellow-500/20">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="font-medium text-brand-text-primary">Attention Needed</span>
              </div>
              <p className="text-sm text-brand-text-secondary">
                321 Elm Street views down 15% - consider refreshing photos or adjusting price
              </p>
            </div>
          </div>
        </DashboardSection>
