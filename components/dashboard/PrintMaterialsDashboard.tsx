...
const PRINT_MATERIAL_TYPES: {
  ...
  },
  {
    ...
    name: "Lawn Sign",
    icon: "🏠",
    description: "Eye-catching yard signs",
  },
  {
    id: "busPanel",
    name: "Bus Panel",
    icon: "🚌",
    description: "Large format advertisements",
  },
  {
    id: "postcard",
    name: "Postcard",
  ...
  },
  {
    ...
    name: "Door Hanger",
    icon: "🚪",
    description: "Neighborhood door hangers",
  },
];
...
const PrintMaterialsDashboard: React.FC = () => {
  ...
  const [selectedType, setSelectedType] = useState<PrintMaterialType | "all">(
    "all",
  );
  const [selectedListing, setSelectedListing] = useState<string>("");
  const [selectedGenerateType, setSelectedGenerateType] =
    useState<PrintMaterialType>("flyer");
  ...
  const summaryMetrics = useMemo(() => {
    ...
    const propertiesWithMaterials = new Set(
      mockPrintMaterials.map((m) => m.listingId),
    ).size;

    // Total materials printed and distributed
    const totalPrintRuns = mockPrintMaterials.reduce(
      (sum, m) => sum + m.printCount,
      0,
    );
    ...
    const typeUsage = mockPrintMaterials.reduce(
      (acc, m) => {
    ...
    const topMaterialType =
      Object.entries(typeUsage).sort(([, a], [, b]) => b - a)[0]?.[0] ||
      "flyer";
  ...
  const getMaterialTypeInfo = (type: PrintMaterialType) => {
    return (
      PRINT_MATERIAL_TYPES.find((t) => t.id === type) || {
        name: type,
        icon: "📄",
      }
    );
  ...
  const getTypeBadgeColor = (type: PrintMaterialType) => {
    ...
    return (
      colorMap[type] || "bg-slate-500/20 text-slate-400 border-slate-500/30"
    );
  };
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-center">
                      <span
                        className={`px-2 py-1 rounded-lg text-xs font-medium border ${getTypeBadgeColor(material.type)}`}
                      >
                    ...
                    <td className="px-6 py-4 text-center">
                      <span className="text-emerald-400 font-bold text-lg">
                        {material.printCount.toLocaleString()}
                      </span>
                      {material.lastPrinted && (
                        <p className="text-slate-400 text-xs">
                          Last: {material.lastPrinted}
                        </p>
                      )}
                    </td>
                    <td className="px-6 py-4 text-center">
                ...
                {material.status === "ready" ? (
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-2">
                      <button
                        onClick={() =>
                          handleCopyDownload(material.downloadUrl, material.id)
                        }
                        className="p-2 rounded-lg bg-white/10 hover:bg-white/20 border border-white/20 transition-all duration-200"
                        title="Copy download link"
                      >
                  ...
                  <div className="bg-white/5 rounded-lg p-4 text-center">
                    ...
                    <p className="text-slate-300 text-sm">
                      Generating {typeInfo.name.toLowerCase()}...
                    </p>
                    <p className="text-slate-400 text-xs">
    ...
    <div className="space-y-8">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-8">
            <div>
              <h2 className="text-3xl font-bold text-white mb-2">
                Print Materials Center
              </h2>
              <p className="text-slate-400">
                Professional marketing materials that drive property visibility
              </p>
            </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-2xl font-bold text-white">
                  {summaryMetrics.propertiesWithMaterials}
                </span>
              </div>
            ...
            <div className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-2xl font-bold text-white">
                  {summaryMetrics.totalPrintRuns.toLocaleString()}
                </span>
              </div>
            ...
            <div className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-2">
                <span className="text-2xl">👥</span>
                <span className="text-2xl font-bold text-white">
                  {(summaryMetrics.estimatedReach / 1000).toFixed(0)}K
                </span>
              </div>
            ...
            <div className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-2">
                <span className="text-2xl">
                  {
                    getMaterialTypeInfo(
                      summaryMetrics.topMaterialType as PrintMaterialType,
                    ).icon
                  }
                </span>
      ...
      {viewMode === "generate" && (
        <div className="relative group">
          ...
          <div className="relative bg-white/5 backdrop-blur-lg border border-white/10 rounded-3xl p-8">
            <h3 className="text-2xl font-bold text-white mb-6">
              Quick Generate Print Materials
            </h3>
              ...
              <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="bg-white/5 rounded-lg p-4">
                        ...
                        <div className="flex items-center space-x-3">
                          <span className="text-lg">
                            {getMaterialTypeInfo(selectedGenerateType).icon}
                          </span>
                          <div>
                      ...
                      <div className="space-y-3">
                        ...
                        <p className="text-slate-400 text-xs text-center">
                          This will open the print generator with your selected
                          options
                        </p>
                      </div>
                    </div>
                ...
                <div className="bg-orange-500/10 border border-orange-500/20 rounded-2xl p-6">
                  ...
                  <div className="space-y-2 text-slate-300 text-sm">
                    <p>
                      <strong>Flyers:</strong> Detailed property sheets for
                      showings
                    </p>
                    <p>
                      <strong>Lawn Signs:</strong> Eye-catching yard displays
                    </p>
                    <p>
                      <strong>Postcards:</strong> Direct mail marketing pieces
                    </p>
                    <p>
                      <strong>Door Hangers:</strong> Neighborhood marketing
                      materials
                    </p>
                    <p>
                      <strong>Bus Panels:</strong> Large-format advertising
                      displays
                    </p>
                  </div>
                </div>
