...
type TabType = 'profile' | 'account' | 'notifications' | 'security' | 'billing' | 'preferences';
      ...
      <div className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-6">
        ...
        <div className="flex items-center space-x-6">
          ...
          <div>
            ...
            <p className="text-slate-400 text-xs mt-1">JPG, PNG or GIF. Max size 5MB</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-6">
      ...
      <div className="space-y-4">
        {Object.entries(notificationSettings).map(([key, value]) => (
          <label key={key} className="flex items-center justify-between p-4 bg-white/5 rounded-lg hover:bg-white/10 transition-colors cursor-pointer">
            <div>
              <p className="text-white font-medium">
                {key.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase())}
              </p>
              <p className="text-slate-400 text-sm mt-1">
                {key === 'emailNotifications' && 'Receive email updates about your listings'}
                {key === 'newListingAlerts' && 'Get notified when you create new listings'}
                {key === 'performanceReports' && 'Weekly performance reports for your properties'}
                {key === 'marketingTips' && 'Tips and best practices for real estate marketing'}
                {key === 'systemUpdates' && 'Important updates about ListingPal features'}
              </p>
            </div>
            <input
              type="checkbox"
  ...
  const renderSecurityTab = () => (
    <div className="space-y-6">
      ...
      <div className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-6">
        ...
        <p className="text-slate-300 mb-4">Add an extra layer of security to your account</p>
        <Button variant="glass" glowColor="emerald">Enable 2FA</Button>
      </div>
    </div>
  );
      ...
      default:
        return (
          <div className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-12 text-center">
            <p className="text-slate-300 text-lg">This section is coming soon!</p>
          </div>
        );
    }
  };
