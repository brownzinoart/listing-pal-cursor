      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {Object.entries(platformMetrics).map(([platform, metrics]) => (
            <div
              key={platform}
              className="bg-brand-background rounded-lg border border-brand-border p-4"
            >
            ...
            <div className="h-48 flex items-end justify-between gap-1">
              {engagementByHour.map((data, index) => {
                const maxEngagement = Math.max(
                  ...engagementByHour.map((d) => d.engagement),
                );
                const height =
                  maxEngagement > 0
                    ? (data.engagement / maxEngagement) * 100
                    : 0;
                ...
                return (
                  ...
                  >
                    <div
                      ...
                      style={{
                        height: `${height}%`,
                        minHeight: data.posts > 0 ? "4px" : "1px",
                      }}
                    />
                    {index % 3 === 0 && (
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-brand-border">
              {topPosts.map((post) => (
                <tr
                  key={post.id}
                  className="hover:bg-brand-panel/50 transition-colors"
                >
