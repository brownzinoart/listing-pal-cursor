...
const SocialPostsAnalytics: React.FC<SocialPostsAnalyticsProps> = ({
  posts,
}) => {
    ...
    Object.keys(metrics).forEach((platform) => {
      ...
      if (metrics[p].posts > 0) {
        metrics[p].avgEngagement = Math.round(
          metrics[p].engagement / metrics[p].posts,
        );
      }
    });
  ...
  const engagementByHour = useMemo(() => {
    const hourData = Array(24)
      .fill(0)
      .map((_, i) => ({ hour: i, posts: 0, engagement: 0 }));
    ...
    hourData.forEach((data) => {
      if (data.posts > 0) {
        data.engagement =
          Math.round((data.engagement / data.posts) * 100) / 100;
      }
    });
            ...
            {bestTimeSlots.length > 0 ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-slate-400 text-sm mt-1">
                  Publish more posts to see optimal timing insights
                </p>
              </div>
            )}
          </div>
        ...
        <div className="relative bg-white/5 backdrop-blur-lg border border-white/10 rounded-3xl p-8">
          ...
          <div className="space-y-4">
            {topPosts.map((post, index) => (
              <div
                key={post.id}
                className="bg-white/5 rounded-2xl p-6 hover:bg-white/10 transition-all duration-200 border-l-4 border-amber-400"
              >
            ...
            {topPosts.length === 0 && (
              <div className="text-center py-12">
                ...
                <p className="text-slate-400 text-sm mt-1">
                  Publish posts to see top performers
                </p>
              </div>
            )}
          </div>
