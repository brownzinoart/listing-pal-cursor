...
const SocialPostsCalendar: React.FC<SocialPostsCalendarProps> = ({
  posts,
  onPostClick,
}) => {
  ...
  const getPlatformIcon = (platform: SocialPost["platform"]) => {
    switch (platform) {
      case "facebook":
        return <FaFacebook className="h-3 w-3 text-blue-600" />;
      case "instagram":
        return <FaInstagram className="h-3 w-3 text-pink-600" />;
      case "twitter":
        return <FaTwitter className="h-3 w-3 text-sky-500" />;
    }
  };
        ...
        <div className="bg-brand-background rounded-lg border border-brand-border p-4">
          ...
          <div className="grid grid-cols-7 gap-1 mb-2">
            {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => (
              <div
                key={day}
                className="text-center text-sm font-medium text-brand-text-secondary py-2"
              >
                {day}
              </div>
            ))}
                  ...
                  <div className="space-y-1">
                    {dayPosts.slice(0, 3).map((post, idx) => (
                      <div key={idx} className="flex items-center gap-1">
                        ...
                        <div
                          className={`h-1 w-full rounded-full ${getStatusColor(post.status)}`}
                        />
                      </div>
                    ))}
                    {dayPosts.length > 3 && (
