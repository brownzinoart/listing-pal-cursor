...
import { DEMO_PROPERTIES, getDemoPropertyWithImages, DemoProperty } from '../../../services/demoPropertyService';
...
const VideoCreationPage: React.FC = () => {
  ...
  const [selectedDemoProperty, setSelectedDemoProperty] = useState<string | null>(null);
          ...
          <div className="space-y-8">
            ...
            <div className="text-center">
              ...
              <p className="text-slate-400 max-w-2xl mx-auto">
                Select a property to create an AI-powered video. Use our demo properties with professional images to see the full experience.
              </p>
            </div>
            ...
            <div className="bg-gradient-to-br from-purple-500/10 to-blue-500/10 border border-purple-500/20 rounded-2xl p-8">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h4 className="text-white font-semibold text-lg">Demo Properties</h4>
              ...
              <p className="text-slate-300 mb-6">
                Try our curated luxury properties with professional photography - perfect for showcasing our AI video generation capabilities.
              </p>
                ...
                {DEMO_PROPERTIES.slice(0, 4).map((property) => (
                  <button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        const { property: demoProperty, images } = await getDemoPropertyWithImages(property.id);
                    ...
                    className={`group relative bg-white/5 border border-white/20 rounded-xl p-6 text-left hover:bg-white/10 hover:border-purple-400/50 transition-all duration-300 ${
                      selectedDemoProperty === property.id ? 'border-purple-400 bg-purple-500/10 ring-2 ring-purple-400/20' : ''
                    }`}
                    ...
                    {selectedDemoProperty === property.id && isLoadingDemo && (
                      <div className="absolute inset-0 bg-black/50 rounded-xl flex items-center justify-center">
                        <div className="text-center">
                          ...
                          <p className="text-sm text-white">Loading property...</p>
                        </div>
                      </div>
                    )}
                  </button>
            ...
            {listings.length > 0 && (
              <div className="bg-white/5 border border-white/20 rounded-2xl p-8">
                <h4 className="text-white font-semibold text-lg mb-6">Your Properties</h4>
                <PropertySelector
                  listings={listings}
            ...
            {listings.length === 0 && !isLoadingListings && (
              <div className="bg-white/5 border border-white/20 rounded-2xl p-8 text-center">
                <div className="text-slate-400 mb-4">
                  ...
                  <p className="text-sm mt-2">Add properties to your portfolio or use our demo properties above</p>
                </div>
              </div>
            )}
          </div>
      ...
      case 'upload':
        return (
          <div className="space-y-8">
            {/* Header */}
            <div className="text-center">
              <h3 className="text-2xl font-bold text-white mb-3">Review Property Details</h3>
              <p className="text-slate-400 max-w-2xl mx-auto">
                Verify your property selection and customize video settings before generating your AI-powered property video.
              </p>
            </div>
            ...
            {selectedListing && (
              <div className="bg-gradient-to-r from-purple-500/10 to-blue-500/10 border border-purple-500/20 rounded-2xl p-6">
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <h4 className="text-white font-semibold">Selected Property</h4>
                </div>
                <div className="bg-white/5 rounded-xl p-4">
                  <h5 className="text-white font-semibold text-lg">{selectedListing.address}</h5>
                  ...
                  <p className="text-2xl font-bold text-purple-400 mt-3">
                    {new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', minimumFractionDigits: 0 }).format(selectedListing.price)}
                  </p>
                </div>
              </div>
            )}
              ...
              <div className="bg-white/5 border border-white/20 rounded-2xl p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-emerald-400 text-sm">{uploadedImages.length} images ready</span>
            ...
            {!selectedDemoProperty && (
              <div className="bg-white/5 border border-white/20 rounded-2xl p-6">
                <h4 className="text-white font-semibold mb-4">Add More Images</h4>
            ...
            {/* Platform Selection */}
            <div className="bg-white/5 border border-white/20 rounded-2xl p-6">
              <h4 className="text-white font-semibold mb-4">Target Platforms</h4>
              <p className="text-slate-400 text-sm mb-6">Choose where you want to publish your video - we'll optimize for each platform</p>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {[
                  { key: 'tiktok', label: 'TikTok', desc: '9:16 vertical • 60s max', icon: '📱' },
                  { key: 'instagram', label: 'Instagram Reels', desc: '9:16 vertical • 90s max', icon: '📷' },
                  { key: 'youtube', label: 'YouTube Shorts', desc: '9:16 vertical • 60s max', icon: '🎥' }
                ].map((platform) => (
                  <button
                    key={platform.key}
            ...
            <div className="flex justify-between items-center">
              ...
              <Button
                ...
                disabled={!selectedListing || uploadedImages.length === 0 || !Object.values(selectedPlatforms).some(Boolean)}
                className="px-8 py-4"
              >
                Generate AI Video
  ...
  const canProceed = () => {
    switch (currentStep) {
      case 'property': return selectedListing !== null;
