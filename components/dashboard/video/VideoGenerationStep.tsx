...
} from "../../shared/VideoProgressIndicator";
import {
  VideoCameraIcon,
  CheckCircleIcon,
  ArrowDownTrayIcon,
    ...
    const generateVideo = async () => {
      try {
        ...
        if (
          isDemoProperty &&
          demoPropertyId &&
          demoVideoService.hasDemoVideo(demoPropertyId)
        ) {
        ...
        setPlatformStatuses((prev) =>
          prev.map((status) => ({
            ...status,
            status: activePlatforms.includes(status.platform)
              ? "processing"
              : "waiting",
          })),
        );
      ...
      <div className="space-y-8">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400">
            Your property video is ready for publishing
          </p>
        </div>
                ...
                generatedVideo.masterVideoUrl.startsWith("/") ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center">
                    <div className="text-center">
                      ...
                      <p className="text-slate-400">
                        Video preview available after processing
                      </p>
                    </div>
                  </div>
                )}
        ...
        <div className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-6">
          ...
          <div className="grid grid-cols-3 gap-4">
            {Object.entries(generatedVideo.platformVersions).map(
              ([platform, version]) =>
                version && (
      ...
      <div className="bg-gradient-to-r from-slate-800/50 to-slate-900/50 backdrop-blur-xl border border-white/10 rounded-3xl p-12">
        <div className="flex justify-center items-center space-x-16">
          {activePlatforms.map((platform, index) => {
            ...
            const platformStatus = platformStatuses.find(
              (s) => s.platform === platform,
            );
            const isActive =
              currentStep >= 2 && platformStatus?.status === "processing";
              ...
              >
                ...
                {isActive && (
                  <div className="absolute inset-0 rounded-2xl opacity-30 animate-pulse">
                    <div
                      className={`w-full h-full bg-gradient-to-r ${data.color} rounded-2xl blur-md`}
                    ></div>
                  </div>
                )}
                ...
                {/* Platform Card */}
                <div
                  className={`relative w-32 h-32 rounded-2xl border-2 transition-all duration-500 ${
                    isComplete
      ...
      <div className="text-center">
        <div className="inline-block bg-gradient-to-r from-blue-500/10 to-purple-500/10 border border-blue-500/20 rounded-2xl px-8 py-4">
          <p className="text-blue-400">
            <span className="font-semibold">✨ AI Magic in Progress:</span>{" "}
            Creating platform-optimized videos for maximum engagement!
          </p>
        </div>
      </div>
    </div>
