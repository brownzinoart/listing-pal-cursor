...
import { VideoGenerationResult, videoGenerationService, PublishResult } from '../../../services/videoGenerationService';
...
const VideoPlatformPublisher: React.FC<VideoPlatformPublisherProps> = ({ 
  ...
  useEffect(() => {
    // Initialize states
    const initialStates: PlatformPublishState[] = activePlatforms.map(platform => ({
      platform,
      status: 'pending'
    }));
  ...
  const publishToAllPlatforms = async () => {
    for (let i = 0; i < activePlatforms.length; i++) {
      ...
      setPublishStates(prev => prev.map(state => 
      ...
      try {
        const result = await videoGenerationService.publishToPlatform(video.videoId, platform);
  ...
  const retryPlatform = async (platform: 'tiktok' | 'instagram' | 'youtube') => {
    setPublishStates(prev => prev.map(state => 
      state.platform === platform
        ? { ...state, status: 'publishing' }
        : state
  ...
  const getPlatformIcon = (platform: string) => {
    switch (platform) {
      case 'tiktok': return <FaTiktok className="h-8 w-8" />;
  ...
  const allSuccessful = publishStates.every(state => state.status === 'success');
  const hasErrors = publishStates.some(state => state.status === 'error');

  return (
    <div className="space-y-8">
      <div className="text-center mb-8">
        <h3 className="text-2xl font-bold text-white mb-2">Publishing Your Video</h3>
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className={`
                  ${state.status === 'publishing' ? 'text-blue-400' : 
                ...
                <div>
                  ...
                  <p className="text-slate-400 text-sm">
                    ...
                    {state.status === 'error' && (state.result?.error || 'Publishing failed')}
                  </p>
                </div>
              </div>
        ...
        <div className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-6">
          ...
          <div className="space-y-2">
            <p className="text-slate-300">
              ...
              </span> of {publishStates.length} platforms published successfully
            </p>
            {hasErrors && (
