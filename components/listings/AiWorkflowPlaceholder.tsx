  ...
  return (
    <div className="w-full flex flex-col items-center">
      <div className="mb-8 mt-12">
        <h3 className="text-xl font-bold text-brand-text-primary text-center mb-6">
            ...
            return (
              ...
              >
                <div className="flex items-center space-x-3 mb-2 min-w-0 w-full">
                  {/* Icon Container */}
                  <div
                    className={`p-2 rounded flex-shrink-0 ${iconBg[tool.id] || "bg-brand-primary"}`}
                  >
                    <Icon
                      className={`w-5 h-5 ${isEnabled ? "text-white" : "text-gray-500"}`}
                    />
                  </div>
                  ...
                  <div className="flex-1 min-w-0 max-w-[160px]">
                    <h4
                      className={`font-medium text-sm ${isEnabled ? "text-brand-text-primary" : "text-gray-500"} truncate`}
                    >
                      {tool.name}
                      {!isEnabled && " (Coming Soon)"}
                    </h4>
                    <p
                      className={`text-xs ${isEnabled ? "text-brand-text-secondary" : "text-gray-600"} truncate`}
                    >
                  ...
                  <div className="flex items-center space-x-1 flex-shrink-0 ml-auto">
                    ...
                    {hasGeneratedContent && isEnabled && (
                      <div className="generated-indicator">
                        ...
                        <span className="text-xs text-green-400 whitespace-nowrap hidden sm:inline">
                          Generated
                        </span>
                      </div>
                    )}
