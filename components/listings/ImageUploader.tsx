...
const ImageUploader: React.FC<ImageUploaderProps> = ({
  ...
  const [images, setImages] =
    useState<Omit<ListingImage, "id">[]>(initialImages);
  ...
  const handleFileChange = useCallback(
    (event: ChangeEvent<HTMLInputElement>) => {
      ...
      Promise.all(newImagesPromises)
        .then((resolvedImages) => {
          const updatedImages = [...images, ...resolvedImages].slice(
            0,
            maxImages,
          );
    ...
    <div className="space-y-4 overflow-hidden">
      <div className="overflow-hidden">
        <label
          ...
          className="block text-sm font-medium text-slate-300 mb-1 break-words"
        >
          Listing Images (up to {maxImages})
        </label>
        <div
          className={`mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-slate-600 border-dashed rounded-md overflow-hidden ${images.length >= maxImages ? "cursor-not-allowed opacity-50" : "hover:border-brand-primary"}`}
        >
