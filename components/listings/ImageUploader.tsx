  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <div className={`mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-slate-600 border-dashed rounded-md ${images.length >= maxImages ? 'cursor-not-allowed opacity-50' : 'hover:border-brand-primary'}`}>
          <div className="space-y-1 text-center">
            <PhotoIcon className="mx-auto h-12 w-12 text-brand-primary" />
            <div className="flex text-sm text-slate-400">
              <label
                htmlFor="image-upload-input"
                className={`relative cursor-pointer bg-slate-800 rounded-md font-medium text-brand-primary hover:text-brand-accent focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-offset-slate-900 focus-within:ring-brand-primary ${images.length >= maxImages ? 'pointer-events-none' : ''}`}
              >
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
            {images.map((image, index) => (
              <div key={image.url.slice(-20) + index} className="relative group aspect-square">
                <img
                  src={image.url}
                ...
                <button
                  ...
                  className="absolute top-1 right-1 bg-red-600 hover:bg-red-700 text-white p-1 rounded-full opacity-0 group-hover:opacity-100 transition-opacity focus:opacity-100"
