...
const detectListingType = (price: number, propertyType?: string): string => {
  ...
  if (propertyType) {
    ...
    if (
      lowerPropType.includes("condo") ||
      lowerPropType.includes("apartment")
    ) {
        ...
        <div className="relative">
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <ChevronDownIcon
                className={`h-4 w-4 text-brand-text-tertiary transition-transform duration-200 ${isOpen ? "rotate-180" : ""} ${disabled ? "opacity-50" : ""}`}
              />
            </div>
          </button>
...
export default function ListingFormPage() {
  ...
  const handleAddressSelect = async (
    ...
    try {
      ...
      if (details) {
        ...
        console.log(
          "  - Estimated Value from API:",
          details.estimatedValue,
          "→ Parsed:",
          parsePrice(details.estimatedValue),
        );
        ...
        console.log(
          ...
          details.bedrooms,
          "→ Parsed:",
          parseNumericValue(details.bedrooms),
        );
        console.log(
          "  - Bathrooms from API:",
          details.bathrooms,
          "→ Parsed:",
          parseNumericValue(details.bathrooms),
        );
        console.log(
          "  - Square Footage from API:",
          details.squareFootage,
          "→ Parsed:",
          parseNumericValue(details.squareFootage),
        );
        console.log(
          "  - Year Built from API:",
          details.yearBuilt,
          "→ Parsed:",
          parseNumericValue(details.yearBuilt),
        );
        ...
        const finalFormData = {
          ...
          bedrooms:
            parseNumericValue(details.bedrooms) || addressFormData.bedrooms,
          bathrooms:
            parseNumericValue(details.bathrooms) || addressFormData.bathrooms,
          sqFt:
            parseNumericValue(details.squareFootage) || addressFormData.sqFt,
          yearBuilt:
            parseNumericValue(details.yearBuilt) || addressFormData.yearBuilt,
      ...
      setFormError(
        `Auto-fill failed: ${error.message || "Could not retrieve all property details."}`,
      );
      console.log(
        "🔄 Auto-fill attempt completed with limited data - check logs above for details",
      );
      setHasAttemptedAutoFill(true);
    } finally {
      setIsFetchingDetails(false);
  ...
  const handleInputChange = (
    ...
    if (
      ["price", "bedrooms", "bathrooms", "sqFt", "yearBuilt"].includes(name)
    ) {
      if (name === "bathrooms") {
  ...
  const handleGenerateAllContent = async () => {
    ...
    if (formData.address.trim().toLowerCase() !== DEMO_ADDRESS.toLowerCase()) {
      setFormError(
        `Content generation is currently only available for the demo address: ${DEMO_ADDRESS}`,
      );
      return;
    }
    ...
    if (
      ...
      setFormError(
        `Please fill in the following required fields before generating content: ${missingFields.join(", ")}.`,
      );
      return;
    }
    ...
    try {
      ...
      setFormError(
        `Failed to save listing: ${err instanceof Error ? err.message : "Unknown error"}`,
      );
    }
  };
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (
      ...
      if (!formData.address) missingFields.push("address");
      if (!formData.price || formData.price <= 0)
        missingFields.push("asking price (must be greater than $0)");
      if (!formData.bedrooms || formData.bedrooms <= 0)
        missingFields.push("bedrooms (must be greater than 0)");
      if (!formData.bathrooms || formData.bathrooms <= 0)
        missingFields.push("bathrooms (must be greater than 0)");
    ...
    try {
      ...
      setFormError(
        `Failed to save the listing: ${err instanceof Error ? err.message : "Unknown error"}`,
      );
    } finally {
      setIsSubmitting(false);
    }
            ...
            <form onSubmit={onSubmit} className="space-y-8">
              ...
              <div className="space-y-6">
                <div className="border-b border-brand-border pb-4">
                  ...
                  <p className="text-sm text-brand-text-tertiary">
                    Primary information required from agent
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="md:col-span-2 space-y-2">
                    ...
                    <p className="text-xs text-brand-text-tertiary">
                      Start typing to search for an address. Property details
                      will auto-fill below.
                    </p>
                  </div>
              ...
              <div className="space-y-6">
                <div className="border-b border-brand-border pb-4">
                  ...
                  <p className="text-sm text-brand-text-tertiary">
                    {isEditing
                      ? "Review and edit the property information"
                      : "Review and edit the auto-filled property information"}
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <EnhancedDropdown
                    ...
                    onChange={(value) =>
                      handleDropdownChange("listingType", value)
                    }
                    options={[
                  ...
                  <div className="bg-brand-accent/10 border border-brand-accent/20 rounded-lg p-4">
                    <div className="flex items-center text-sm">
                      ...
                      <span className="text-brand-text-secondary">
                        Property data auto-filled from:{" "}
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                    {uploadedImages.map((file, index) => (
                      <div key={index} className="relative group">
                        ...
                        <p className="text-sm text-brand-text-tertiary mt-2 text-center">
                          Image {index + 1}
                        </p>
                      </div>
                    ))}
              ...
              <div className="space-y-3 mt-8">
                ...
                <label className="block text-brand-text-secondary text-sm font-medium">
                  Anything else we should know? Add details that will help us
                  generate richer marketing content for this listing.
                </label>
                <textarea
                  name="keyFeatures"
              ...
              <div className="flex justify-end space-x-4 pt-8 border-t border-brand-border">
                ...
                <Button
                  ...
                  leftIcon={
                    !isSubmitting ? <Save className="h-5 w-5" /> : undefined
                  }
                >
                  {isSubmitting
                    ? "Saving..."
        ...
        <div className="mt-6 p-4 bg-brand-panel/50 border border-brand-border/50 rounded-lg">
          <div className="flex items-start space-x-2">
            <div className="flex-shrink-0 mt-0.5">
              <svg
                className="w-4 h-4 text-brand-text-tertiary"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
            ...
            <div className="text-xs text-brand-text-tertiary leading-relaxed">
              <p className="mb-1">
                <strong>Data Sources:</strong> Information displayed comes from
                Google Places API, Google Custom Search API, public real estate
                listings, and AI-enhanced insights when real data is limited.
              </p>
              <p>
                Listing details are auto-filled from available public listings
                and may not reflect current conditions. Always verify
                information independently for accuracy.
              </p>
            </div>
          </div>
        </div>
