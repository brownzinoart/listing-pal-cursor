...
export const LocationContextWidget: React.FC<LocationContextWidgetProps> = ({
  ...
  const fetchLocationContext = async () => {
    ...
    try {
      ...
      if (latitude && longitude) {
        ...
        console.log(`🌍 Fetching real data for ${address} at coordinates (${latitude}, ${longitude})`);
      } else {
        console.log(`📍 Fetching data for ${address} (no coordinates available)`);
      }
  ...
  if (loading) {
    return (
      <div className={`bg-gradient-to-r from-brand-card to-brand-panel rounded-xl p-6 border border-brand-border ${className}`}>
        <div className="flex items-center space-x-4">
          <div className="relative">
            <div className="w-8 h-8 border-4 border-brand-border border-t-brand-primary rounded-full animate-spin"></div>
          </div>
          <div>
            <h3 className="text-lg font-semibold text-brand-text-primary">Analyzing Neighborhood</h3>
  ...
  if (error) {
    return (
      <div className={`bg-brand-card rounded-xl p-6 border border-brand-danger/30 ${className}`}>
        <div className="flex items-start space-x-3">
          <div className="flex-shrink-0">
            <svg className="h-6 w-6 text-brand-danger" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
          </div>
          <div className="flex-1">
            <h3 className="text-sm font-medium text-brand-text-primary">Unable to fetch neighborhood data</h3>
      ...
      <div className="mb-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4">
          <div className="mb-4 sm:mb-0">
            <h3 className="text-2xl font-bold text-brand-text-primary mb-2">Neighborhood Insights</h3>
            <div className="flex items-center">
              <span className="bg-brand-secondary h-2 w-2 rounded-full mr-2"></span>
              <span className="text-sm font-medium text-brand-secondary">Automatically enhancing your listing</span>
            </div>
          </div>
        </div>
      </div>
            ...
            cards.length > 0 && (
              <div key={category} className="space-y-4 w-full">
                ...
                <div className="flex items-center space-x-4 py-3 sticky top-0 bg-brand-card z-10 border-b border-brand-border">
                  ...
                  <span className="text-sm text-gray-700 font-semibold bg-gray-200 px-3 py-1 rounded-full">{cards.length} insights</span>
                </div>
...
const ContextCard: React.FC<{
  ...
  return (
    ...
    >
      ...
      <div className={`absolute top-3 right-3 w-5 h-5 rounded-full border-2 transition-all z-10 ${
        isSelected 
      ...
      <div className="p-6 w-full h-full overflow-hidden">
        ...
        {/* Card content */}
        <p className="text-gray-700 text-sm mb-4 leading-relaxed w-full" style={{
          display: '-webkit-box',
          WebkitLineClamp: 2,
        ...
        <div className={`space-y-2 w-full ${isExpanded ? 'max-h-32 overflow-y-auto' : ''}`}>
          {card.preview.bullets.slice(0, isExpanded ? undefined : 2).map((bullet, idx) => (
            <div key={idx} className="flex items-start space-x-3 text-sm w-full">
              ...
              <span className="flex-1 text-gray-600 leading-relaxed">{bullet}</span>
            </div>
          ))}
        </div>
