...
const PDFExportButton: React.FC<PDFExportButtonProps> = ({
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      // Update progress
      toast.loading('Generating PDF from captured content...', { id: loadingToast });
      
      console.log('📄 Generating PDF with html2canvas...');
      // Use PDFExportService to capture the pdf-export-content element
      await PDFExportService.exportListingToPDF(listing, DEFAULT_AGENT, 'pdf-export-content');
      ...
      if (error instanceof Error) {
        ...
        if (error.message.includes('download')) {
          ...
          suggestion = 'There was an issue capturing the page content. Please try again.';
        } else if (error.message.includes('Element not found')) {
          suggestion = 'Content container not found. Please refresh the page and try again.';
        }
      }
