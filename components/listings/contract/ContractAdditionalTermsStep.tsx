...
const ContractAdditionalTermsStep: React.FC<
  ContractAdditionalTermsStepProps
> = ({ listing, contractData, onUpdate, onNext, onPrevious }) => {
  ...
  const standardAddenda = [
    {
      ...
      description:
        "This offer is contingent upon termination of existing contract",
    },
    {
  ...
  useEffect(() => {
    if (addenda.length === 0) {
      ...
      if (
        contractData.financials?.financingType === "fha" ||
        contractData.financials?.financingType === "va"
      ) {
        initialAddenda.push({
          type: "fha-va",
          included: true,
  ...
  const updateAddendum = (
    index: number,
    updates: Partial<ContractAddendum>,
  ) => {
  ...
  const addStipulation = (stipulation: string) => {
    if (
      stipulation.trim() &&
      !specialStipulations.includes(stipulation.trim())
    ) {
    ...
    <div className="space-y-8">
      ...
      <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-6">
        ...
        <div className="text-blue-300 text-sm space-y-2">
          <p>
            <strong>Special Stipulations</strong> are custom terms specific to
            this transaction that supplement the standard contract language.
          </p>
          <p>
            <strong>Addenda</strong> are standardized additional forms that
            address specific situations like HOA properties, government
            financing, or short sales.
          </p>
          <p>
            <strong>Additional Terms</strong> provide space for any other
            contract modifications or clarifications.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-xl p-6">
        ...
        <div className="mb-6">
          <h4 className="text-white font-medium mb-3">
            Available Addenda (click to add)
          </h4>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {standardAddenda
              .filter(
                (standard) => !addenda.some((a) => a.type === standard.type),
              )
              .map((standard) => (
                <button
                  key={standard.type}
                ...
                <div className="flex items-center gap-3">
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input
                      ...
                      onChange={(e) =>
                        updateAddendum(index, { included: e.target.checked })
                      }
              ...
              {addendum.included && (
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      updateAddendum(index, { details: e.target.value })
                    }
      ...
      <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-xl p-6">
        ...
        <div className="space-y-4">
          ...
          <div>
            <h4 className="text-white font-medium mb-3">
              Common Stipulations (click to add)
            </h4>
            <div className="flex flex-wrap gap-2">
              {commonStipulations
                .filter(
                  (stipulation) => !specialStipulations.includes(stipulation),
                )
          ...
          <div className="flex gap-2">
            <input
              ...
              onKeyPress={(e) =>
                e.key === "Enter" && addStipulation(newStipulation)
              }
            />
            <Button
              variant="glass"
          ...
          {specialStipulations.length > 0 && (
            <div className="space-y-2">
              ...
              {specialStipulations.map((stipulation, index) => (
                <div
                  key={index}
                  className="flex items-start justify-between bg-slate-800/50 rounded-lg p-3"
                >
                  <span className="text-white text-sm flex-1 mr-2">
                    {stipulation}
                  </span>
                  <button
      ...
      <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-xl p-6">
        ...
        <div>
          ...
          <p className="text-xs text-slate-400 mt-2">
            Use this space for any additional contract language, modifications
            to standard terms, or specific agreements between parties.
          </p>
        </div>
      </div>
      ...
      <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-6">
        ...
        <div className="text-amber-300 text-sm space-y-2">
          <p>
            <strong>Attorney Review Required:</strong> All additional terms,
            stipulations, and addenda should be reviewed by a qualified North
            Carolina real estate attorney before contract execution.
          </p>
          <p>
            <strong>Compliance:</strong> Ensure all terms comply with North
            Carolina real estate law and local regulations.
          </p>
          <p>
            <strong>Clarity:</strong> Use clear, unambiguous language. Avoid
            conflicting terms that may contradict the standard contract
            language.
          </p>
        </div>
      </div>
        ...
        <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-6">
          ...
          <div className="text-emerald-300 text-sm space-y-2">
            {addenda.filter((a) => a.included).length > 0 && (
              <p>
                • {addenda.filter((a) => a.included).length} addenda will be
