...
const ContractDatesStep: React.FC<ContractDatesStepProps> = ({
  ...
  const calculateSuggestedDates = (offerDate: string) => {
    ...
    return {
      offerExpirationDate: new Date(offer.getTime() + 2 * 24 * 60 * 60 * 1000)
        .toISOString()
        .split("T")[0], // 2 days
      dueDiligenceDeadline: new Date(offer.getTime() + 21 * 24 * 60 * 60 * 1000)
        .toISOString()
        .split("T")[0], // 21 days (NC standard)
      closingDate: new Date(offer.getTime() + 45 * 24 * 60 * 60 * 1000)
        .toISOString()
        .split("T")[0], // 45 days
      possessionDate: new Date(offer.getTime() + 45 * 24 * 60 * 60 * 1000)
        .toISOString()
        .split("T")[0], // Same as closing
      inspectionDeadline: new Date(offer.getTime() + 10 * 24 * 60 * 60 * 1000)
        .toISOString()
        .split("T")[0], // 10 days
      financingDeadline: new Date(offer.getTime() + 21 * 24 * 60 * 60 * 1000)
        .toISOString()
        .split("T")[0], // Same as due diligence
      appraisalDeadline: new Date(offer.getTime() + 21 * 24 * 60 * 60 * 1000)
        .toISOString()
        .split("T")[0], // Same as due diligence
    };
  };
  ...
  useEffect(() => {
    if (
      ...
      setDates((prev) => ({
        ...prev,
        offerExpirationDate:
          prev.offerExpirationDate || suggested.offerExpirationDate,
        dueDiligenceDeadline:
          prev.dueDiligenceDeadline || suggested.dueDiligenceDeadline,
        closingDate: prev.closingDate || suggested.closingDate,
        possessionDate: prev.possessionDate || suggested.possessionDate,
        inspectionDeadline:
          prev.inspectionDeadline || suggested.inspectionDeadline,
        financingDeadline:
          prev.financingDeadline || suggested.financingDeadline,
        appraisalDeadline:
          prev.appraisalDeadline || suggested.appraisalDeadline,
      }));
    }
  }, []);
  ...
  const validationErrorsMemo = useMemo(() => {
    ...
    if (
      dates.offerExpirationDate &&
      dates.offerExpirationDate <= dates.offerDate
    ) {
    ...
    if (
      dates.dueDiligenceDeadline &&
      dates.dueDiligenceDeadline <= dates.offerDate
    ) {
    ...
    if (
      dates.closingDate &&
      dates.dueDiligenceDeadline &&
      dates.closingDate <= dates.dueDiligenceDeadline
    ) {
      errors.push("Closing date must be after due diligence deadline");
    }

    if (
      dates.possessionDate &&
      dates.closingDate &&
      dates.possessionDate < dates.closingDate
    ) {
  ...
  const updateDate = useCallback(
    (field: keyof ContractDates, value: string) => {
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-xl p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                onChange={(e) =>
                  updateDate("offerExpirationDate", e.target.value)
                }
                className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
                required
              />
            ...
            <p className="text-xs text-slate-400 mt-1">
              {dates.offerExpirationDate &&
                `${formatDateForDisplay(dates.offerExpirationDate)} (${getDateDifference(dates.offerDate, dates.offerExpirationDate)})`}
            </p>
          </div>
          ...
          <div className="md:col-span-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="relative">
                <input
                  ...
                  onChange={(e) =>
                    updateDate("dueDiligenceDeadline", e.target.value)
                  }
                  className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
                  required
                />
            ...
            <p className="text-xs text-slate-400 mt-1">
              {dates.dueDiligenceDeadline &&
                `Ends: ${formatDateForDisplay(dates.dueDiligenceDeadline)} at 5:00 PM`}
            </p>
          </div>
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">
              {dates.closingDate &&
                `${formatDateForDisplay(dates.closingDate)} (${getDateDifference(dates.offerDate, dates.closingDate)})`}
            </p>
          </div>
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">
              When buyer receives keys and possession
            </p>
          </div>
        </div>
      </div>
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                onChange={(e) =>
                  updateDate("inspectionDeadline", e.target.value)
                }
                className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
              />
              <button
            ...
            <p className="text-xs text-slate-400 mt-1">
              Professional inspections must be completed
            </p>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                onChange={(e) =>
                  updateDate("financingDeadline", e.target.value)
                }
                className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
              />
              <button
            ...
            <p className="text-xs text-slate-400 mt-1">
              Loan approval must be obtained
            </p>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                onChange={(e) =>
                  updateDate("appraisalDeadline", e.target.value)
                }
                className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
              />
              <button
            ...
            <p className="text-xs text-slate-400 mt-1">
              Property appraisal must be completed
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-6">
        ...
        <div className="space-y-3 text-sm text-amber-300">
          <p>
            <strong>Unique to NC:</strong> During the due diligence period, the
            buyer has an unrestricted right to terminate the contract for any
            reason or no reason at all.
          </p>
          <p>
            <strong>Timeline:</strong> Typically 21-30 days, giving buyers time
            to conduct inspections, secure financing, and review all aspects of
            the property.
          </p>
          <p>
            <strong>Key Point:</strong> All other contingency deadlines should
            fall within or align with the due diligence deadline for maximum
            buyer protection.
          </p>
        </div>
      </div>
            ...
            <div className="flex justify-between items-center">
              <div className="text-center">
                ...
                <p className="text-xs text-slate-400">
                  {formatDateForDisplay(dates.offerDate)}
                </p>
              </div>
              <div className="text-center">
                ...
                <p className="text-xs text-slate-400">
                  {formatDateForDisplay(dates.dueDiligenceDeadline)}
                </p>
              </div>
              <div className="text-center">
                ...
                <p className="text-xs text-slate-400">
                  {formatDateForDisplay(dates.closingDate)}
                </p>
              </div>
            </div>
