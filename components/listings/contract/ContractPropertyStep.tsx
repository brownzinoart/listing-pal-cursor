...
const ContractPropertyStep: React.FC<ContractPropertyStepProps> = ({
  ...
  const removeIncludedItem = (index: number) => {
    updateProperty('includedItems', property.includedItems.filter((_, i) => i !== index));
  };
  ...
  const removeExcludedItem = (index: number) => {
    updateProperty('excludedItems', property.excludedItems.filter((_, i) => i !== index));
  };

  const addKnownDefect = () => {
    if (newDefect.trim() && !property.knownDefects?.includes(newDefect.trim())) {
      updateProperty('knownDefects', [...(property.knownDefects || []), newDefect.trim()]);
      setNewDefect('');
    }
  };

  const removeKnownDefect = (index: number) => {
    updateProperty('knownDefects', property.knownDefects?.filter((_, i) => i !== index) || []);
  };
    ...
    <div className="space-y-8">
      ...
      {validationErrors.length > 0 && (
        <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-4">
          <div className="flex items-start space-x-3">
            ...
            <div>
              <h4 className="text-red-400 font-semibold mb-2">Property Information Issues:</h4>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <input
              ...
              required
            />
            <p className="text-xs text-slate-400 mt-1">Required for NC property records</p>
          </div>
          ...
          <div className="md:col-span-2">
            ...
            <textarea
              ...
              onChange={(e) => updateProperty('legalDescription', e.target.value)}
              className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
              placeholder="Legal description from deed or survey"
              rows={3}
            />
            <p className="text-xs text-slate-400 mt-1">Detailed legal description helps ensure accurate property identification</p>
          </div>
        </div>
      </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              {[
                ...
                <button
                  key={condition.value}
                  onClick={() => updateProperty('propertyCondition', condition.value)}
                  className={`
                ...
                >
                  ...
                  <div className="text-xs text-slate-400 mt-1">{condition.desc}</div>
                </button>
              ))}
            </div>
          </div>
      ...
      <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-xl p-6">
        ...
        <div className="space-y-4">
          ...
          <div>
            <h4 className="text-white font-medium mb-3">Standard Items (click to add)</h4>
          ...
          <div className="flex gap-2">
            <input
              ...
              onKeyPress={(e) => e.key === 'Enter' && addIncludedItem(newIncludedItem)}
            />
            <Button
              variant="glass"
              size="sm"
          ...
          <div className="space-y-2">
            {property.includedItems.map((item, index) => (
              <div key={index} className="flex items-center justify-between bg-emerald-500/10 border border-emerald-500/20 rounded-lg p-3">
      ...
      {/* Excluded Items */}
      <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-xl p-6">
        <h3 className="text-lg font-semibold text-white mb-6">Items Excluded from Sale</h3>

        <div className="space-y-4">
          {/* Quick add common exclusions */}
          <div>
            <h4 className="text-white font-medium mb-3">Common Exclusions (click to add)</h4>
            <div className="flex flex-wrap gap-2">
              {commonExcludedItems
          ...
          <div className="flex gap-2">
            <input
              ...
              onKeyPress={(e) => e.key === 'Enter' && addExcludedItem(newExcludedItem)}
            />
            <Button
              variant="glass"
              size="sm"
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-amber-300 text-sm">Required for homes built before 1978</p>
            </div>
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                type="checkbox"
                checked={property.leadPaintDisclosure || false}
                onChange={(e) => updateProperty('leadPaintDisclosure', e.target.checked)}
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="text-white font-medium">Property Disclosure Statement</p>
              <p className="text-amber-300 text-sm">NC residential property disclosure</p>
            </div>
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                type="checkbox"
                checked={property.propertyDisclosureProvided || false}
                onChange={(e) => updateProperty('propertyDisclosureProvided', e.target.checked)}
                className="sr-only peer"
              />
              <div className="w-11 h-6 bg-slate-600 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-amber-500"></div>
            </label>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-amber-300 text-sm">Transfer of subsurface mineral rights</p>
            </div>
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                type="checkbox"
                checked={property.mineralRightsIncluded || false}
                onChange={(e) => updateProperty('mineralRightsIncluded', e.target.checked)}
                className="sr-only peer"
              />
              <div className="w-11 h-6 bg-slate-600 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-amber-500"></div>
            </label>
