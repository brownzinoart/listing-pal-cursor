...
const ContractReviewStep: React.FC<ContractReviewStepProps> = ({
  ...
  const handleGenerateContract = async () => {
    ...
    try {
      ...
      const preview = ContractGenerationService.formatContractForDisplay(
        finalContract,
        template,
      );
  ...
  const getSectionSummary = (
    ...
    <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-xl p-4">
      <div className="flex justify-between items-center mb-3">
        ...
        <button
          onClick={() =>
            onJumpToStep(sectionTitle.toLowerCase().replace(" ", ""))
          }
  ...
  return (
    <div className="space-y-8">
      {/* Validation Status */}
      <div
        className={`rounded-xl p-4 ${validationResult.isValid ? "bg-emerald-500/10 border border-emerald-500/20" : "bg-red-500/10 border border-red-500/20"}`}
      >
        <div className="flex items-start space-x-3">
          ...
          <div>
            <h3
              className={`font-semibold ${validationResult.isValid ? "text-emerald-400" : "text-red-400"}`}
            >
              {validationResult.isValid
            ...
            {validationResult.isValid ? (
              <p className="text-emerald-300 text-sm mt-1">
                All required information has been provided. You can generate
                your North Carolina purchase contract.
              </p>
            ) : (
              <div className="mt-2">
                <p className="text-red-300 text-sm mb-2">
                  Please address the following issues before generating:
                </p>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {getSectionSummary("Parties", contractData, [
            {
              label: "Buyers",
              value: contractData.buyers
                ?.map((b) => `${b.firstName} ${b.lastName}`)
                .join(", "),
            },
            {
              label: "Sellers",
              value: contractData.sellers
                ?.map((s) => `${s.firstName} ${s.lastName}`)
                .join(", "),
            },
            {
              label: "Buyer Attorney",
              value: contractData.buyerAttorney
                ? `${contractData.buyerAttorney.firstName} ${contractData.buyerAttorney.lastName}`
                : "None specified",
            },
          ])}
          ...
          {getSectionSummary("Financial Terms", contractData.financials, [
            {
              ...
              value: contractData.financials?.purchasePrice
                ? formatCurrency(contractData.financials.purchasePrice)
                : "Not set",
            },
            {
              label: "Earnest Money",
              value: contractData.financials?.earnestMoneyDeposit
                ? formatCurrency(contractData.financials.earnestMoneyDeposit)
                : "Not set",
            },
            {
              label: "Financing Type",
              value: contractData.financials?.financingType,
            },
            {
              label: "Down Payment",
              value: contractData.financials?.downPaymentAmount
                ? formatCurrency(contractData.financials.downPaymentAmount)
                : "Not specified",
            },
          ])}
          ...
          {getSectionSummary("Important Dates", contractData.dates, [
            {
              label: "Offer Date",
              value: formatDate(contractData.dates?.offerDate || ""),
            },
            {
              label: "Due Diligence Deadline",
              value: formatDate(contractData.dates?.dueDiligenceDeadline || ""),
            },
            {
              label: "Closing Date",
              value: formatDate(contractData.dates?.closingDate || ""),
            },
            {
              label: "Possession Date",
              value: formatDate(contractData.dates?.possessionDate || ""),
            },
          ])}
        </div>
      </div>
      ...
      <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-6">
        <h4 className="text-amber-400 font-semibold mb-4">
          North Carolina Contract Features
        </h4>
        ...
        {!showPreview ? (
          <div className="text-center space-y-4">
            <p className="text-slate-300">
              Ready to generate your North Carolina Offer to Purchase and
              Contract. This will create a legally formatted document based on
          ...
          <div className="space-y-4">
            ...
            <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-lg p-4">
              <p className="text-emerald-300 text-sm">
                <strong>Next Steps:</strong> Review the generated contract
                carefully. This document should be reviewed by your attorney
                before presentation to ensure it meets all legal requirements
