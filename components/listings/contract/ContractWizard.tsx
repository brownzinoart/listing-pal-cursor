...
// Define the steps
const WIZARD_STEPS = [
  { id: 'basic', title: 'Basic Information', component: 'ContractBasicInfoStep' },
  { id: 'parties', title: 'Parties', component: 'ContractPartiesStep' },
  { id: 'financial', title: 'Financial Terms', component: 'ContractFinancialStep' },
  { id: 'dates', title: 'Dates & Deadlines', component: 'ContractDatesStep' },
  { id: 'property', title: 'Property Details', component: 'ContractPropertyStep' },
  { id: 'contingencies', title: 'Contingencies', component: 'ContractContingenciesStep' },
  { id: 'additional', title: 'Additional Terms', component: 'ContractAdditionalTermsStep' },
...
const ContractWizard: React.FC = () => {
    ...
    listingService.getListingById(listingId)
      .then(data => {
        if (data && data.userId === user?.id) {
          ...
          const initialData = ContractGenerationService.createContractFromListing(data);
          ...
          setError(data ? "You don't have permission to create a contract for this listing." : "Listing not found.");
        }
      })
      .catch(err => {
  ...
  useEffect(() => {
    if (stepId) {
      ...
      if (listingId) {
        navigate(`/listings/${listingId}/contract/${WIZARD_STEPS[0].id}`, { replace: true });
      }
    }
    ...
    switch (step.id) {
      ...
      case 'parties':
        ...
        if (contractData.state === 'NC' && !contractData.buyerAttorney) {
          errors.push('Buyer attorney information is required for North Carolina contracts');
        }
        break;
        
      case 'financial':
        // Financial validation
        if (!contractData.financials?.purchasePrice || contractData.financials.purchasePrice <= 0) {
          errors.push('Purchase price must be greater than 0');
        }
        if (!contractData.financials?.earnestMoneyDeposit || contractData.financials.earnestMoneyDeposit <= 0) {
      ...
      case 'dates':
        ...
        if (contractData.state === 'NC' && !contractData.dates?.dueDiligenceDeadline) {
      ...
      case 'contingencies':
        ...
        const includedContingencies = contractData.contingencies?.filter(c => c.included) || [];
        includedContingencies.forEach((contingency, index) => {
          if (contingency.type === 'sale' && !contingency.propertyAddress) {
            errors.push(`Sale contingency ${index + 1} requires property address`);
          }
        });
        break;
      ...
      case 'additional':
        ...
        const includedAddenda = contractData.addenda?.filter(a => a.included) || [];
  ...
  if (isLoading) {
    return (
      <ModernDashboardLayout title="Contract Wizard" subtitle="Loading contract wizard...">
  ...
  if (error) {
    return (
      <ModernDashboardLayout title="Contract Wizard" subtitle="Error loading contract wizard">
      ...
      <div className="relative group mb-8">
        ...
        <div className="relative bg-white/5 backdrop-blur-lg border border-white/10 rounded-3xl p-6">
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <span className="text-sm font-semibold text-amber-400">{Math.round(progressPercentage)}% Complete</span>
          </div>
