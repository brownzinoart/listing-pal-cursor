...
const ContractorLocationTimelineStep: React.FC<
  ContractorLocationTimelineStepProps
> = ({ listing, searchCriteria, onUpdate, onNext, onPrevious }) => {
  ...
  const canContinue = Boolean(
    searchCriteria.searchRadius && searchCriteria.timeframe,
  );
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
        ...
        <p className="text-slate-400">
          Set your search area and project timeline for{" "}
          {searchCriteria.projectType?.replace("-", " ")} work
        </p>
      </div>
      ...
      <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
        <h4 className="text-white font-semibold mb-4">
          How far should we search for contractors?
        </h4>
        ...
        {searchCriteria.searchRadius && (
          <div className="mt-4 p-3 bg-blue-500/10 border border-blue-500/20 rounded-lg">
            <p className="text-blue-400 text-sm">
              ✓ Searching within {searchCriteria.searchRadius} miles of{" "}
              ...
              <span className="text-white">
                Timeline:{" "}
                {
                  TIMEFRAME_OPTIONS.find(
                    (t) => t.value === searchCriteria.timeframe,
                  )?.label
                }
              </span>
            </div>
