...
const WIZARD_STEPS = [
  { id: 'scope', title: 'Project Scope', component: 'ContractorProjectScopeStep' },
  { id: 'location', title: 'Location & Timeline', component: 'ContractorLocationTimelineStep' },
  { id: 'requirements', title: 'Requirements', component: 'ContractorRequirementsStep' },
  { id: 'results', title: 'Search Results', component: 'ContractorResultsStep' },
  { id: 'selection', title: 'Compare & Select', component: 'ContractorSelectionStep' },
  { id: 'booking', title: 'Request Quote', component: 'ContractorBookingStep' },
];

const ContractorSearchWizard: React.FC = () => {
  const { id: listingId, stepId } = useParams<{ id: string; stepId?: string }>();
  ...
  // Search data state
  const [searchCriteria, setSearchCriteria] = useState<Partial<ContractorSearchCriteria>>({
  ...
  const [quoteRequest, setQuoteRequest] = useState<ServiceQuoteRequest | null>(null);
    ...
    listingService.getListingById(listingId)
      .then(data => {
        if (data && data.userId === user?.id) {
          ...
          const initialCriteria = ContractorSearchService.createSearchCriteriaFromListing(data);
          ...
          setError(data ? "You don't have permission to search contractors for this listing." : "Listing not found.");
        }
      })
      .catch(err => {
  ...
  useEffect(() => {
    if (stepId) {
      ...
      if (listingId) {
        navigate(`/listings/${listingId}/contractor/${WIZARD_STEPS[0].id}`, { replace: true });
      }
    }
  ...
  const handleNext = async () => {
    ...
    if (currentStepIndex === 2) { // requirements step
      await performSearch();
    }
  ...
  const performSearch = async () => {
    try {
      setIsSearching(true);
      const results = await ContractorSearchService.searchContractors(searchCriteria as ContractorSearchCriteria);
  ...
  const validateCurrentStep = async (): Promise<{ isValid: boolean; errors: string[] }> => {
    ...
    switch (step.id) {
      case 'scope':
        if (!searchCriteria.tradeNeeded || searchCriteria.tradeNeeded.length === 0) {
      ...
      case 'requirements':
        if (!searchCriteria.budgetRange || searchCriteria.budgetRange.min <= 0 || searchCriteria.budgetRange.max <= searchCriteria.budgetRange.min) {
      ...
      case 'results':
        if (searchResults.length === 0) {
          errors.push('No contractors found. Please adjust your criteria and search again.');
        }
        break;
  ...
  const handleContractorSelection = async (contractorId: string) => {
    try {
      const contractor = await ContractorSearchService.getContractorById(contractorId);
      if (contractor) {
  ...
  const handleQuoteRequest = async (quoteData: Partial<ServiceQuoteRequest>) => {
    try {
      if (!selectedContractor) {
  ...
  if (isLoading) {
    return (
      <ModernDashboardLayout title="Contractor Search" subtitle="Loading contractor search wizard...">
  ...
  if (error) {
    return (
      <ModernDashboardLayout title="Contractor Search" subtitle="Error loading contractor search wizard">
      ...
      <div className="relative group mb-8">
        ...
        <div className="relative bg-white/5 backdrop-blur-lg border border-white/10 rounded-3xl p-6">
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <span className="text-sm font-semibold text-orange-400">{Math.round(progressPercentage)}% Complete</span>
          </div>
