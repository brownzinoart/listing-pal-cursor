...
const ContractorSelectionStep: React.FC<ContractorSelectionStepProps> = ({
  ...
  const [selectedForDetails, setSelectedForDetails] = useState<string | null>(selectedContractor?.id || null);
  ...
  const detailsContractor = selectedForDetails 
    ? searchResults.find(r => r.provider.id === selectedForDetails)?.provider as Contractor
    : null;
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
        <h3 className="text-lg font-semibold text-white mb-2">Compare & Select Contractor</h3>
        <p className="text-slate-400">
          Review detailed information and select your preferred contractor for quotes
        </p>
            ...
            return (
              ...
              >
                ...
                <div className="text-center mb-6">
                  ...
                  <p className="text-slate-400 text-sm mt-1">({contractor.totalReviews} reviews)</p>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between items-center py-2 border-b border-white/10">
                    ...
                    <span className="text-white font-semibold">
                      ${contractor.insurance[0]?.coverageAmount.toLocaleString() || 'N/A'}
                    </span>
                  </div>
                  ...
                  <div className="flex justify-between items-center py-2">
                    ...
                    <span className="text-white font-semibold">
                      {new Date(contractor.availability.nextAvailableDate).toLocaleDateString()}
                    </span>
                  </div>
                </div>
                ...
                {/* Specialties */}
                <div className="mt-6">
                  <p className="text-slate-400 text-xs mb-2">Top Specialties:</p>
      ...
      {activeTab === 'details' && detailsContractor && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
              <h4 className="text-white font-semibold mb-4">Select Contractor</h4>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <p className="text-white font-semibold text-sm">{contractor.businessName}</p>
                          <div className="flex items-center space-x-1">
                            {renderStarRating(contractor.overallRating)}
                          </div>
                        </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
              <div className="flex items-start justify-between mb-4">
                <div>
                  <h3 className="text-xl font-bold text-white">{detailsContractor.businessName}</h3>
                  <p className="text-slate-400">{detailsContractor.contactName}</p>
                  <div className="flex items-center space-x-4 mt-2 text-sm">
                    <span className="text-slate-300">{detailsContractor.city}, {detailsContractor.state}</span>
                    <span className="text-slate-300">•</span>
                    <span className="text-slate-300">{detailsContractor.yearsInBusiness} years experience</span>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-slate-300 text-sm">{detailsContractor.phone}</span>
                </div>
                <div className="flex items-center space-x-2">
                  <EnvelopeIcon className="h-4 w-4 text-slate-400" />
                  <span className="text-slate-300 text-sm">{detailsContractor.email}</span>
                </div>
            ...
            <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
              <h4 className="text-white font-semibold mb-4">Certifications & Insurance</h4>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <h5 className="text-slate-300 font-medium mb-3">Licenses & Certifications</h5>
                  <div className="space-y-3">
                    {detailsContractor.certifications.map((cert) => (
                      <div key={cert.id} className="bg-white/5 rounded-lg p-3">
                        ...
                        <p className="text-slate-400 text-xs">{cert.issuingBody}</p>
                        <p className="text-slate-400 text-xs">
                          {cert.certificationNumber && `#${cert.certificationNumber} • `}
                          Valid until {cert.expirationDate ? new Date(cert.expirationDate).toLocaleDateString() : 'N/A'}
                        </p>
                      </div>
                    ))}
                  </div>
                ...
                <div>
                  <h5 className="text-slate-300 font-medium mb-3">Insurance Coverage</h5>
                  <div className="space-y-3">
                ...
                {detailsContractor.reviews.slice(0, 3).map((review) => (
                  <div key={review.id} className="bg-white/5 rounded-lg p-4">
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        <p className="text-white font-medium text-sm">{review.reviewerName}</p>
                    ...
                    {review.verified && (
                      <p className="text-emerald-400 text-xs mt-2">✓ Verified Project</p>
                    )}
                  </div>
                ))}
              </div>
      ...
      {selectedContractor && (
        <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-2xl p-6">
          <div className="flex items-center justify-between">
            <div>
              <h4 className="text-emerald-400 font-semibold">Ready to Request Quote</h4>
              <p className="text-white">
                Selected: <span className="font-semibold">{selectedContractor.businessName}</span>
              </p>
              <p className="text-slate-400 text-sm">
                {formatPrice(selectedContractor.hourlyRate)}/hr • Available {new Date(selectedContractor.availability.nextAvailableDate).toLocaleDateString()}
              </p>
            </div>
            <div className="flex items-center space-x-2">
