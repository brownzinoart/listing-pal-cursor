...
const SocialPostMockup: React.FC<
  Omit<AdCreativeMockupProps, "platform"> & {
    platform: "facebook" | "linkedin";
  }
> = ({ listing, headline, body, cta, platform }) => {
  const primaryImage =
    listing.images && listing.images.length > 0
      ? listing.images[0].url
      : `https://via.placeholder.com/600x400?text=${encodeURIComponent(listing.address.split(",")[0])}`;
    ...
    <div className="w-full max-w-sm mx-auto bg-brand-card border border-brand-border rounded-lg shadow-lg p-0 font-sans">
      <div className="p-3">
        <div className="flex items-center mb-3">
          <div
            className={`h-10 w-10 rounded-full flex items-center justify-center mr-3 shrink-0 ${config.iconBg}`}
          >
            {config.icon}
          </div>
          <div>
        ...
        <p className="text-sm text-brand-text-secondary mb-3 whitespace-pre-line break-words">
          {body ||
            "Your engaging ad copy will appear here. It should be compelling enough to make users stop scrolling!"}
        </p>
      </div>
      <div className="bg-brand-panel border-t border-b border-brand-border cursor-pointer">
        ...
        />
        <div className="p-3">
          <p className="text-xs text-brand-text-tertiary uppercase">
            yourwebsite.com
          </p>
          <h4 className="font-semibold text-brand-text-primary truncate">
...
// Google Ad Mockup
const GoogleAdMockup: React.FC<Omit<AdCreativeMockupProps, "platform">> = ({
  headline,
  body,
  cta,
  listing,
    ...
    <div className="w-full max-w-sm mx-auto bg-brand-card border border-brand-border rounded-lg shadow-lg p-4 font-sans">
      <div className="flex items-center mb-2">
        ...
        <div>
          ...
          <p className="text-xs text-brand-text-tertiary">
            Ad · https://www.yourwebsite.com/listings/{listing.id || "123"}
          </p>
        </div>
      </div>
      ...
      <p className="text-sm text-brand-text-secondary">
        {body ||
          "Find your dream home with us. We have a wide range of properties available for you."}
      </p>
    </div>
  );
};
