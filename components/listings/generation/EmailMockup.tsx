...
const EmailMockup: React.FC<EmailMockupProps> = ({ emailContent }) => {
  ...
  const extractSubjectAndBody = (content: string) => {
    ...
    if (subjectLineIndex >= 0) {
      ...
      if (subjectLine.includes('Dear') || subjectLine.includes('Hello') || subjectLine.includes('Hi ') || subjectLine.length > 80) {
        ...
        if (sentenceEnd > 0 && sentenceEnd < 60) {
          ...
          bodyContent = extraContent + (remainingLines.length > 0 ? '\n' + remainingLines.join('\n') : '');
        } else {
    ...
    <div className="w-full max-w-lg mx-auto bg-white rounded-lg shadow-lg overflow-hidden">
      ...
      <div className="p-4">
        <div className="text-sm text-gray-700 space-y-3 max-h-80 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100">
          {bodyContent.split('\n').map((line, index) => (
            <p key={index} className={`${line.trim() === '' ? 'h-3' : ''} leading-relaxed`}>
              {line.trim() || '\u00A0'}
            </p>
          ))}
        </div>
      ...
      <div className="px-4 py-3 bg-gray-50 border-t border-gray-200">
        <div className="flex items-center justify-between">
          ...
          <div className="flex space-x-2">
            <button className="text-xs text-teal-600 hover:text-teal-700">Reply</button>
            <button className="text-xs text-teal-600 hover:text-teal-700">Forward</button>
          </div>
        </div>
      </div>
    </div>
