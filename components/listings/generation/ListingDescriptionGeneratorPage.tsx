...
const ListingDescriptionGeneratorPage: React.FC = () => {
  ...
  const [selectedStyle, setSelectedStyle] = useState<DescriptionStyleId>(
    DESCRIPTION_STYLES[0].id,
  );
  ...
  const handleGenerateDescription = async () => {
    ...
    try {
      ...
      const response = await ContentGenerationService.generateDescription(
        propertyData,
        backendStyle as any,
      );
  ...
  const handleConfirmAndSave = async () => {
    ...
    if (
      generatedDescription ===
        "Your generated description will appear here..." ||
      generatedDescription === "Generating your content..."
    ) {
  ...
  const handleCopyDescription = () => {
    ...
    navigator.clipboard
      .writeText(generatedDescription)
