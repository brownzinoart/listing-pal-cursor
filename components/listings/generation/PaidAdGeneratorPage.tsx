...
const AD_OBJECTIVES: { id: AdObjective; name: string; description: string }[] =
  [
    ...
    },
    {
      ...
      description: "Collect contact info with an instant form.",
    },
    {
      id: "BRAND_AWARENESS",
      name: "Brand Awareness",
      description: "Maximize the number of people who see your ad.",
    },
  ];
...
const PaidAdGeneratorPage: React.FC = () => {
  ...
  const [selectedObjective, setSelectedObjective] =
    useState<AdObjective>("WEBSITE_TRAFFIC");
  const [generatedAds, setGeneratedAds] = useState<Record<string, AdCopy>>({});
  const [finalizedPlatforms, setFinalizedPlatforms] = useState<Set<AdPlatform>>(
    new Set(),
  );
  const [isGenerating, setIsGenerating] = useState(false);

  const currentAdCopy = useMemo(
    () => generatedAds[activePlatform],
    [generatedAds, activePlatform],
  );
  const isCurrentPlatformFinalized = useMemo(
    () => finalizedPlatforms.has(activePlatform),
    [finalizedPlatforms, activePlatform],
  );

  // Character remaining calculations
  const platformLimits = PLATFORM_LIMITS[activePlatform];
  const headlineRemaining =
    platformLimits.headline - (currentAdCopy?.headline?.length || 0);
  const bodyRemaining =
    platformLimits.body - (currentAdCopy?.body?.length || 0);
  const ctaRemaining = platformLimits.cta - (currentAdCopy?.cta?.length || 0);

  // Best practices for current platform and objective
  const currentBestPractices =
    PLATFORM_OBJECTIVE_BEST_PRACTICES[activePlatform][selectedObjective];
  ...
  const handleGenerateAdCopy = async () => {
    ...
    try {
      const objective =
        AD_OBJECTIVES.find((o) => o.id === selectedObjective)?.name ||
        "Website Traffic";
      ...
      const adCopy = await ollamaService.generateAdCopy(
        listing,
        activePlatform,
        enhancedObjective,
      );
  ...
  const getPlatformObjectiveContext = () => {
    const contexts = {
      facebook: {
        WEBSITE_TRAFFIC:
          "Focus on driving clicks to the property listing. Use casual, engaging language that encourages immediate action. Highlight unique property features and create urgency.",
        LEAD_GENERATION:
          "Optimize for collecting contact information through Facebook's lead forms. Emphasize free consultations, property tours, or market reports as lead magnets.",
        BRAND_AWARENESS:
          "Build recognition for the real estate agent/agency. Focus on expertise, local market knowledge, and successful track record. Use storytelling elements.",
      },
      linkedin: {
        WEBSITE_TRAFFIC:
          "Target real estate professionals and investors. Use professional language and focus on investment opportunities, market insights, or commercial properties.",
        LEAD_GENERATION:
          "Generate B2B leads for commercial real estate or referral partnerships. Emphasize professional credentials, market expertise, and business solutions.",
        BRAND_AWARENESS:
          "Establish thought leadership in real estate. Focus on market analysis, professional achievements, and industry expertise.",
      },
      google: {
        WEBSITE_TRAFFIC:
          "Capture high-intent search traffic. Use specific location keywords, property types, and price ranges. Match search intent closely.",
        LEAD_GENERATION:
          "Target users actively searching for properties. Emphasize immediate availability, contact information, and competitive advantages.",
        BRAND_AWARENESS:
          "Build visibility for local real estate searches. Focus on local expertise, service areas, and unique value propositions.",
      },
    };
  ...
  const getPreviousStepPath = () => {
    ...
    if (currentIndex > 0) {
      ...
      return previousTool?.pathSuffix
        ? `/listings/${listingId}${previousTool.pathSuffix}?workflow=${workflowParam}`
        : `/listings/${listingId}`;
    }
                ...
                {AD_PLATFORMS.map((platform) => (
                  ...
                  >
                    <div className="flex items-center">
                      <platform.icon
                        className={`h-5 w-5 mr-3 flex-shrink-0 ${activePlatform === platform.id ? "text-brand-primary" : "text-brand-text-secondary"}`}
                      />
                      <span className="font-medium text-brand-text-primary text-sm">
                        {platform.name}
                      </span>
                    </div>
                    {finalizedPlatforms.has(platform.id) && (
              ...
              {isCurrentPlatformFinalized ? (
                <div className="text-center p-4 bg-green-500/10 rounded-lg border border-green-500/20">
                  ...
                  <p className="font-semibold text-green-400 text-sm">
                    "{AD_PLATFORMS.find((p) => p.id === activePlatform)?.name}"
                    ad is finalized!
                  </p>
                  <p className="text-xs text-green-500/80 mt-1">
                    Select another platform to continue.
                  </p>
                </div>
              ) : (
                <Button
                  ...
                  leftIcon={
                    !currentAdCopy ? (
                      <SparklesOutlineIcon className="h-5 w-5" />
                    ) : (
            ...
            <div className="bg-brand-panel p-6 rounded-lg border border-brand-border">
              <div className="flex justify-between items-center mb-6">
                ...
                <div className="text-sm text-brand-text-secondary">
                  ...
                  <span className="font-medium text-brand-text-primary">
                    {
                      AD_OBJECTIVES.find((o) => o.id === selectedObjective)
                        ?.name
                    }
                  </span>
                </div>
              ...
              <div className="mt-8 pt-6 border-t border-brand-border flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div className="text-sm text-brand-text-secondary">
                  {finalizedPlatforms.size > 0 ? (
                    <span className="text-green-500 font-medium">
                      ✓ {finalizedPlatforms.size} ad
                      {finalizedPlatforms.size !== 1 ? "s" : ""} finalized
                    </span>
                  ) : (
                ...
                >
                  Save{" "}
                  {finalizedPlatforms.size > 0 ? finalizedPlatforms.size : ""}{" "}
                  Ad{finalizedPlatforms.size !== 1 ? "s" : ""}
                </Button>
              </div>
            </div>
          </div>
