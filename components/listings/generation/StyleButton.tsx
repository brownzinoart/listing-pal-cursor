...
const StyleButton: React.FC<StyleButtonProps> = ({ name, description, isSelected, onClick }) => {
  return (
    <button
      type="button"
      onClick={onClick}
    ...
    >
      <h4 className={`font-semibold text-brand-text-primary ${isSelected ? 'text-brand-secondary' : ''}`}>{name}</h4>
