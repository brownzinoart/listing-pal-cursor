...
const XMockup: React.FC<XMockupProps> = ({
  ...
  return (
    <div className="w-full max-w-md mx-auto bg-brand-card border border-brand-border rounded-lg shadow-lg p-4">
      <div className="flex items-start mb-2">
        ...
        <div className="flex-1">
          <div className="flex items-center">
            <p className="font-bold text-sm text-brand-text-primary mr-1">
              {profileName}
            </p>
          ...
          {/* Post Text */}
          <p
            className={`text-sm text-brand-text-secondary whitespace-pre-line break-words ${charLimitExceeded ? "text-red-400" : ""}`}
          >
            {postText}
          </p>
        </div>
      ...
      {maxLength && (
        <p
          className={`text-xs mt-2 text-right ${charLimitExceeded ? "text-red-500 font-semibold" : "text-brand-text-tertiary"}`}
        >
          {charCount}/{maxLength}
        </p>
      )}
