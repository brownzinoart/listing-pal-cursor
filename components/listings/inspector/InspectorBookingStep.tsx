      ...
      <div className="space-y-8">
        ...
        <div className="bg-emerald-500/20 border border-emerald-500/30 rounded-2xl p-8">
          <div className="text-center">
            ...
            <h3 className="text-2xl font-bold text-white mb-2">Inspection Booked Successfully!</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-white font-semibold">
                    {appointment.scheduledTime} ({appointment.estimatedDuration || 180} minutes)
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-white font-semibold">
                    {formatPrice(appointment.estimatedCost || inspector.standardInspectionFee)}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-white font-semibold capitalize">{appointment.contactMethod}</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center space-x-1 mt-1">
                  ...
                  <span className="text-slate-400 text-sm ml-2">({inspector.totalReviews} reviews)</span>
                </div>
              </div>
        ...
        <div className="bg-blue-500/10 border border-blue-500/20 rounded-2xl p-6">
          ...
          <div className="space-y-3">
            <div className="flex items-start space-x-3">
              ...
              <div>
                <p className="text-white font-medium text-sm">Confirmation Call/Email</p>
                <p className="text-slate-400 text-xs">You'll receive confirmation within 2 hours via your preferred contact method</p>
              </div>
            </div>
            ...
            <div className="flex items-start space-x-3">
              ...
              <div>
                <p className="text-white font-medium text-sm">Pre-Inspection Checklist</p>
                <p className="text-slate-400 text-xs">Receive a checklist to prepare your property for inspection</p>
              </div>
            </div>
            ...
            <div className="flex items-start space-x-3">
              ...
              <div>
                ...
                <p className="text-slate-400 text-xs">Inspector arrives on time and conducts thorough inspection</p>
              </div>
            </div>
            ...
            <div className="flex items-start space-x-3">
              ...
              <div>
                <p className="text-white font-medium text-sm">Detailed Report</p>
                <p className="text-slate-400 text-xs">Receive comprehensive report within {inspector.reportTurnaroundHours} hours</p>
              </div>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
        <h3 className="text-lg font-semibold text-white mb-4">Book Your Inspection</h3>
        <div className="flex items-center space-x-4">
          <div className="w-12 h-12 rounded-xl bg-blue-500/20 flex items-center justify-center">
            <span className="text-blue-400 text-lg font-bold">
              {inspector.businessName[0]}
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center space-x-4 mt-1">
              <div className="flex items-center space-x-1">
                ...
                <span className="text-slate-400 text-sm ml-2">{inspector.overallRating}</span>
              </div>
              {inspector.verified && (
          ...
          <div className="text-right">
            ...
            <p className="text-slate-400 text-sm">{listing.city}, {listing.state}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
        ...
        <div className="space-y-3">
          ...
          >
            <div>
              <p className="text-white font-semibold">General Home Inspection</p>
              <p className="text-slate-400 text-sm">Comprehensive inspection of all major systems</p>
            </div>
            <div className="text-right">
              <p className="text-emerald-400 font-bold">{formatPrice(inspector.standardInspectionFee)}</p>
              <p className="text-slate-400 text-xs">Included</p>
            </div>
          </div>

          {inspector.additionalServiceFees.map((service, index) => {
            const serviceId = service.serviceName.toLowerCase().replace(/\s+/g, '-');
            ...
            return (
              ...
              >
                <div>
                  <p className="text-white font-semibold">{service.serviceName}</p>
                  <p className="text-slate-400 text-sm">Additional specialized inspection</p>
                </div>
                <div className="text-right">
                  <p className="text-emerald-400 font-bold">+{formatPrice(service.fee)}</p>
                  <p className="text-slate-400 text-xs">{isSelected ? 'Selected' : 'Optional'}</p>
                </div>
              </div>
            );
          })}
      ...
      <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
        ...
        <div>
          <label className="block text-slate-300 text-sm mb-2">Special Instructions (Optional)</label>
          <textarea
            value={clientNotes}
      ...
      <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-center py-2 border-b border-white/10">
            ...
            <span className="text-white font-semibold">{formatPrice(inspector.standardInspectionFee)}</span>
          </div>
          
          {selectedServices.filter(s => s !== 'general').map((serviceId) => {
            const service = inspector.additionalServiceFees.find(s => 
              s.serviceName.toLowerCase().includes(serviceId.replace('-', ' '))
            );
            if (!service) return null;
            
            return (
              <div key={serviceId} className="flex justify-between items-center py-2 border-b border-white/10">
                <span className="text-slate-300">{service.serviceName}</span>
                <span className="text-white font-semibold">+{formatPrice(service.fee)}</span>
              </div>
            );
          })}
          ...
          <div className="flex justify-between items-center py-3 text-lg">
            ...
            <span className="text-emerald-400 font-bold">{formatPrice(calculateTotalCost())}</span>
          </div>
        </div>
      </div>
