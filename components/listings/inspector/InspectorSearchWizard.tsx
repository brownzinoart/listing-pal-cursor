...
const WIZARD_STEPS = [
  { id: 'criteria', title: 'Search Criteria', component: 'InspectorSearchCriteriaStep' },
  { id: 'location', title: 'Location & Timing', component: 'InspectorLocationStep' },
  { id: 'requirements', title: 'Requirements', component: 'InspectorRequirementsStep' },
  { id: 'results', title: 'Search Results', component: 'InspectorResultsStep' },
  { id: 'selection', title: 'Compare & Select', component: 'InspectorSelectionStep' },
  { id: 'booking', title: 'Book Inspection', component: 'InspectorBookingStep' },
];
...
const InspectorSearchWizard: React.FC = () => {
  ...
  // Search data state
  const [searchCriteria, setSearchCriteria] = useState<Partial<InspectorSearchCriteria>>({
    ...
    listingService.getListingById(listingId)
      .then(data => {
        if (data && data.userId === user?.id) {
          ...
          const initialCriteria = InspectorSearchService.createSearchCriteriaFromListing(data);
          ...
          setError(data ? "You don't have permission to search inspectors for this listing." : "Listing not found.");
        }
      })
      .catch(err => {
  ...
  useEffect(() => {
    if (stepId) {
      ...
      if (listingId) {
        navigate(`/listings/${listingId}/inspector/${WIZARD_STEPS[0].id}`, { replace: true });
      }
    }
  ...
  const performSearch = async () => {
    try {
      setIsSearching(true);
      const results = await InspectorSearchService.searchInspectors(searchCriteria as InspectorSearchCriteria);
  ...
  const validateCurrentStep = async (): Promise<{ isValid: boolean; errors: string[] }> => {
    ...
    switch (step.id) {
      case 'criteria':
        if (!searchCriteria.inspectionTypes || searchCriteria.inspectionTypes.length === 0) {
      ...
      case 'results':
        if (searchResults.length === 0) {
          errors.push('No inspectors found. Please adjust your criteria and search again.');
        }
        break;
  ...
  const handleInspectorSelection = async (inspectorId: string) => {
    try {
      const inspector = await InspectorSearchService.getInspectorById(inspectorId);
      if (inspector) {
  ...
  const handleAppointmentBooking = async (appointmentData: Partial<ServiceAppointment>) => {
    try {
      if (!selectedInspector) {
  ...
  if (isLoading) {
    return (
      <ModernDashboardLayout title="Inspector Search" subtitle="Loading inspector search wizard...">
  ...
  if (error) {
    return (
      <ModernDashboardLayout title="Inspector Search" subtitle="Error loading inspector search wizard">
      ...
      <div className="relative group mb-8">
        ...
        <div className="relative bg-white/5 backdrop-blur-lg border border-white/10 rounded-3xl p-6">
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <span className="text-sm font-semibold text-blue-400">{Math.round(progressPercentage)}% Complete</span>
          </div>
