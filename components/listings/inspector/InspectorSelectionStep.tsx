...
const InspectorSelectionStep: React.FC<InspectorSelectionStepProps> = ({
  ...
  const [selectedForDetails, setSelectedForDetails] = useState<string | null>(selectedInspector?.id || null);
  ...
  const detailsInspector = selectedForDetails 
    ? searchResults.find(r => r.provider.id === selectedForDetails)?.provider as Inspector
    : null;
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
        <h3 className="text-lg font-semibold text-white mb-2">Compare & Select Inspector</h3>
            ...
            return (
              ...
              >
                ...
                <div className="text-center mb-6">
                  ...
                  <p className="text-slate-400 text-sm mt-1">({inspector.totalReviews} reviews)</p>
                </div>
                ...
                <div className="mt-6">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {inspector.specialties.slice(0, 2).map((specialty, index) => (
                      <span
                        key={index}
      ...
      {activeTab === 'details' && detailsInspector && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
              <h4 className="text-white font-semibold mb-4">Select Inspector</h4>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <p className="text-white font-semibold text-sm">{inspector.businessName}</p>
                          <p className="text-slate-400 text-xs">{renderStarRating(inspector.overallRating)}</p>
                        </div>
                      </div>
                    </div>
                  );
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
              <div className="flex items-start justify-between mb-4">
                <div>
                  <h3 className="text-xl font-bold text-white">{detailsInspector.businessName}</h3>
                  <p className="text-slate-400">{detailsInspector.contactName}</p>
                  <div className="flex items-center space-x-4 mt-2 text-sm">
                    <span className="text-slate-300">{detailsInspector.city}, {detailsInspector.state}</span>
                    <span className="text-slate-300">•</span>
                    <span className="text-slate-300">{detailsInspector.yearsInBusiness} years experience</span>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-slate-300 text-sm">{detailsInspector.phone}</span>
                </div>
                <div className="flex items-center space-x-2">
                  <EnvelopeIcon className="h-4 w-4 text-slate-400" />
                  <span className="text-slate-300 text-sm">{detailsInspector.email}</span>
                </div>
            ...
            <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <h5 className="text-slate-300 font-medium mb-3">Certifications</h5>
                  <div className="space-y-3">
              ...
              <div className="space-y-4">
                {detailsInspector.reviews.slice(0, 3).map((review) => (
                  <div key={review.id} className="bg-white/5 rounded-lg p-4">
                    ...
                    {review.verified && (
                      <p className="text-emerald-400 text-xs mt-2">✓ Verified Purchase</p>
                    )}
                  </div>
                ))}
              </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-white">
                Selected: <span className="font-semibold">{selectedInspector.businessName}</span>
              </p>
              <p className="text-slate-400 text-sm">
                {formatPrice(selectedInspector.standardInspectionFee)} • Available {new Date(selectedInspector.availability.nextAvailableDate).toLocaleDateString()}
              </p>
            </div>
            <div className="flex items-center space-x-2">
              <CalendarIcon className="h-5 w-5 text-emerald-400" />
              <span className="text-emerald-400 font-semibold">Schedule Inspection</span>
            </div>
          </div>
        </div>
      )}
