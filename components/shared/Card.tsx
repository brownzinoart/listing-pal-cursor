...
const Card: React.FC<CardProps> = ({
  ...
  const hoverClasses = hover
    ? "hover:shadow-brand-lg hover:scale-[1.02] transform transition-all duration-200"
  ...
  return (
    <div className={cardClasses} onClick={onClick}>
      {header && (
        <div className="px-6 py-4 border-b border-brand-border">{header}</div>
      )}
      <div className={contentClasses}>{children}</div>
      {footer && (
        <div className="px-6 py-4 border-t border-brand-border">{footer}</div>
      )}
    </div>
  );
};
