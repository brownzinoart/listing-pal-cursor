...
const NeighborhoodInsights: React.FC<NeighborhoodInsightsProps> = ({
    ...
    try {
      ...
      const transformedData = {
        ...
        marketTrends: {
          ...
          priceGrowth5Year: 0,
          rentYield:
            marketTrends.medianRent && marketTrends.medianPrice
              ? ((marketTrends.medianRent * 12) / marketTrends.medianPrice) *
                100
              : 0,
        },
        ...
        demographics: {
          medianAge: 0,
          medianIncome: 0,
          familyFriendly: Math.min(
            10,
            Math.max(
              0,
              schools.length * 2 + (insights.walkability?.walk || 0) / 10,
            ),
          ),
          diversityIndex: 0,
          population: 0,
        ...
        investmentMetrics: {
          ...
          risks: ["Market volatility potential", "Interest rate sensitivity"],
        },
        ...
        lifestyleScore: {
          ...
          dining: Math.min(
            ...
            amenities.filter(
              (a) =>
                a.category.includes("restaurant") ||
                a.category.includes("cafe"),
            ).length * 15,
          ),
          shopping: Math.min(
            100,
            amenities.filter(
              (a) =>
                a.category.includes("shopping") || a.category.includes("store"),
            ).length * 20,
          ),
          recreation: Math.min(
            100,
            amenities.filter(
              (a) => a.category.includes("park") || a.category.includes("gym"),
            ).length * 25,
          ),
          nightlife: 0,
          familyFriendly: Math.min(
            100,
            schools.length * 20 + (insights.walkability?.walk || 0) / 2,
          ),
        },
  ...
  const tabs = [
    ...
    },
    {
      id: "schools",
      label: "Schools",
      tooltip: "School ratings & appeal",
      icon: <GraduationCap className="w-4 h-4" />,
    },
    {
      id: "amenities",
      label: "Lifestyle",
      tooltip: "Walkability & amenities",
      icon: <ShoppingCart className="w-4 h-4" />,
    },
    {
      id: "market",
      label: "Market",
      tooltip: "Pricing & trends",
      icon: <Wallet className="w-4 h-4" />,
    },
    {
      id: "neighborhood",
      label: "Neighborhood",
      tooltip: "Safety & community profile",
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-sm text-brand-text-secondary mt-1">
                  Safety statistics and community insights
                </p>
              </div>
              {!viewMode && (
                <button
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-bold text-green-800">
                      {data.demographics.familyFriendly}/10
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                ...
                <ul className="space-y-2 text-sm">
                  <li className="flex items-start space-x-2 text-blue-700">
                    ...
                    <span>
                      Emphasize safety statistics for buyer confidence
                    </span>
                  </li>
                  <li className="flex items-start space-x-2 text-blue-700">
                    <span className="text-blue-500 mt-1">•</span>
