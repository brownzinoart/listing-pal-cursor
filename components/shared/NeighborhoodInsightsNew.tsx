...
${data.transportation.commuteOptions
  .slice(0, 5)
  .map(
    (option) =>
      `• ${option.name} - ${option.categories?.[0] || "Transportation"}`,
  )
  .join("\n")}
                ...
                <div className="w-20 bg-gray-200 rounded-full h-2">
                  <div
                    className={`h-2 rounded-full ${
                      data.overview.dataQuality > 70
                        ? "bg-green-500"
                        : data.overview.dataQuality > 40
                          ? "bg-yellow-500"
                          : "bg-red-500"
                    }`}
          ...
          {data.places.shopping.length > 0 && (
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {data.places.shopping
                  .slice(0, 6)
                  .map((shop) => renderListItem(shop, "Retail"))}
              </div>
            </div>
          )}
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">
                      {data.safety.crimeData.crimeRate}/100k residents
                    </span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium capitalize">
                      {data.safety.crimeData.comparedToNational}
                    </span>
                  </div>
                </div>
              </div>
        ...
        <div className="space-y-6">
          ...
          {data.transportation.commuteOptions.length > 0 && (
            <div>
              <h4 className="text-lg font-semibold text-gray-900 mb-4">
                ...
                {data.transportation.commuteOptions.length})
              </h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          {data.transportation.transportationBenefits.length > 0 && (
            <div>
              ...
              <div className="space-y-2">
                {data.transportation.transportationBenefits.map(
                  (benefit, index) => (
                    <div
                      key={index}
                      className="flex items-center space-x-2 p-3 bg-green-50 rounded-lg"
