...
const PortfolioSummary: React.FC<PortfolioSummaryProps> = ({
  ...
  const avgPropertyValue =
    totalProperties > 0 ? totalValue / totalProperties : 0;
        ...
        <div className="lg:col-span-8">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-white/80 text-sm">
                {totalProperties} properties • {formatPrice(totalValue)} total
                value
                {activeFilterCount > 0 && (
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="bg-white/10 rounded-lg p-4">
              ...
              <div className="text-xl font-bold text-white">
                {performanceStats.avgConversionRate.toFixed(1)}%
              </div>
            </div>
            ...
            <div className="bg-white/10 rounded-lg p-4">
              ...
              <div className="text-xl font-bold text-white">
                {performanceStats.avgDaysOnMarket} days
              </div>
            </div>
            ...
            <div className="bg-white/10 rounded-lg p-4">
              ...
              <div className="text-xl font-bold text-white">
                {performanceStats.totalLeads.toLocaleString()}
              </div>
            </div>
          </div>
        </div>
                    ...
                    (activeFilters.priceRange.min ||
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-white/80 text-xs">
                          Price:{" "}
                          {activeFilters.priceRange.min
                            ? formatPrice(activeFilters.priceRange.min)
                            : "$0"}{" "}
                          -{" "}
                          {activeFilters.priceRange.max
                            ? formatPrice(activeFilters.priceRange.max)
                            : "No limit"}
                        </span>
                      </div>
                    )}
                  ...
                  {activeFilters.propertyTypes &&
                    activeFilters.propertyTypes.length > 0 && (
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-white/80 text-xs">
                          Types:{" "}
                          {activeFilters.propertyTypes.slice(0, 2).join(", ")}
                          {activeFilters.propertyTypes.length > 2 &&
                            ` +${activeFilters.propertyTypes.length - 2} more`}
                        </span>
                      </div>
                    )}
                  ...
                  {activeFilters.locations &&
                    activeFilters.locations.length > 0 && (
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-white/80 text-xs">
                          Locations:{" "}
                          {activeFilters.locations.slice(0, 2).join(", ")}
                          {activeFilters.locations.length > 2 &&
                            ` +${activeFilters.locations.length - 2} more`}
                        </span>
                      </div>
                    )}
                </div>
