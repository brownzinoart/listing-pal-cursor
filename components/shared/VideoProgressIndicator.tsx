...
export const PlatformProgress: React.FC<PlatformProgressProps> = ({ statuses }) => {
  const platformIcons = {
  ...
  const getStatusColor = (status: PlatformStatus['status']) => {
    switch (status) {
      case 'waiting': return 'text-slate-400';
          ...
          return (
            <div key={index} className="flex items-center gap-4">
              <div className={`
                ...
                ${isComplete ? 'bg-gradient-to-r from-purple-500 to-pink-500' : 
                  isActive ? 'bg-white/20 border-2 border-purple-400' : 
                  'bg-white/10 border-2 border-white/20'}
              `}>
                {isComplete ? (
                  <CheckCircleIcon className="h-6 w-6 text-white" />
                ) : (
                  <span className={`text-sm font-bold ${isActive ? 'text-purple-400' : 'text-slate-400'}`}>
                    {index + 1}
                  </span>
                )}
                {isActive && (
