...
const PieChart: React.FC<PieChartProps> = ({
      ...
      return (
        <div className="bg-brand-panel border border-brand-border rounded-lg p-3 shadow-lg">
          ...
          <div className="text-brand-text-secondary text-sm mt-1">
            ...
            <span className="text-brand-text-primary font-medium">
              {formatTooltip
                ? formatTooltip(data.value, data.name, data)
                : data.value}
            </span>
          </div>
  ...
  return (
    ...
    >
      <div
        className={`flex ${legendPosition === "left" ? "flex-row" : legendPosition === "right" ? "flex-row-reverse" : "flex-col"} items-center justify-center h-full`}
      >
