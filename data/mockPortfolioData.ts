...
export const mockListings: Listing[] = [
  ...
  // Investment Properties
  {
    ...
    price: 1950000,
    keyFeatures:
      "Student rental income, UC Berkeley proximity, duplex potential",
    images: [],
    propertyType: "single-family",
    listingType: "sale",
    status: "active",
  ...
  },
  {
    ...
    price: 2250000,
    keyFeatures:
      "Beach access, vacation rental potential, Spanish architecture",
    images: [],
    propertyType: "single-family",
    listingType: "sale",
    status: "coming_soon",
...
export const generateTimeSeriesData = () => {
  ...
  return months.map((month, index) => {
    ...
    const seasonalMultiplier =
      month.includes("Dec") || month.includes("Jan") ? 0.6 : 1.0;
    const springBoost =
      month.includes("Mar") || month.includes("Apr") || month.includes("May")
        ? 1.4
        : 1.0;
    const summerSustain =
      month.includes("Jun") || month.includes("Jul") ? 1.2 : 1.0;
    ...
    return {
      ...
      portfolioValue: Math.round(
        (18500000 + index * 500000) * (0.95 + Math.random() * 0.1),
...
export const generatePropertyPerformanceData = () => {
  return mockListings.map((listing, index) => {
    ...
    let baseLeads =
      priceRange === "luxury" ? 6 : priceRange === "premium" ? 4 : 3;
    ...
    if (forceNeedsAttention && listing.status === "active") {
      ...
      monthlyLeads = Math.round(
        baseLeads * marketBoost * statusBoost * (0.7 + Math.random() * 0.6),
      );
    }

    const monthlyShowings = Math.round(
      monthlyLeads * (0.6 + Math.random() * 0.3),
    );
    const offers =
      listing.status === "sold"
    ...
    const getActionNeeded = () => {
      if (listing.id === "listing-010")
        return "Price reduction: 5-8% below market value";
      if (listing.id === "listing-016")
        return "Update photos: Current images are 6+ months old";
      if (listing.id === "listing-018")
        return "Marketing refresh: New copy & social media push";
      if (listing.id === "listing-004")
        return "Seasonal pricing: Adjust for fall market conditions";
      if (listing.id === "listing-008")
        return "Professional staging: Living areas need furniture";

      // General rules for other properties
      if (daysOnMarket > 75 && monthlyLeads < 1)
        return "Major price reduction: 10-15% recommended";
      if (daysOnMarket > 60 && monthlyLeads < 2)
        return "Price reduction: 5-8% recommended";
      if (daysOnMarket > 45 && monthlyLeads < 2)
        return "Fresh marketing campaign & new photos";
      if (daysOnMarket > 30 && monthlyLeads < 1)
        return "Professional staging & virtual tour";
      if (monthlyLeads > 0 && monthlyShowings === 0)
        return "Follow up with leads: Convert inquiries to showings";
      return null;
    };
    ...
    return {
      ...
      pricePerSqFt: Math.round(
        (listing.price || 0) / (listing.squareFootage || 1),
      ),
...
export const getMarketSegments = () => {
  ...
  const midMarket = mockListings.filter(
  ...
  return {
    ...
    midMarket: {
      ...
      avgPrice:
        midMarket.reduce((sum, l) => sum + l.price, 0) / midMarket.length,
    ...
    entryLevel: {
      ...
      avgPrice: entryLevel.length
        ? entryLevel.reduce((sum, l) => sum + l.price, 0) / entryLevel.length
        : 0,
...
export const getGeographicData = () => {
  ...
  const californiaCities = californiaProperties.reduce(
    (acc, listing) => {
      if (!acc[listing.city]) {
      ...
      acc[listing.city].avgPrice =
        acc[listing.city].totalValue / acc[listing.city].count;
      return acc;
    },
    {} as Record<
      string,
  ...
  return {
    ...
    outOfState: outOfStateProperties.reduce(
      (acc, listing) => {
...
export const generateAIInsights = () => {
  ...
  return {
    topPerformer: {
      ...
      recommendation:
        "Consider replicating this listing's marketing strategy across similar properties.",
    },
    marketTiming: {
      type: "info",
      title: "Market Timing",
      insight:
        "Properties priced above $2M are selling 25% faster than market average.",
      recommendation:
        "Luxury properties are in high demand - consider strategic pricing for quick sales.",
    },
    attentionNeeded:
      needsAttention.length > 0
        ? {
            ...
            recommendation:
              "Review photography quality and competitive pricing for these properties.",
          }
        : null,
    trafficSurge:
      highTraffic.length > 0
        ? {
            ...
            recommendation:
              "Monitor these high-traffic properties for potential quick sales opportunities.",
          }
        : null,
  };
};
