...
async function createNovaWithAudio() {
  ...
  try {
    ...
    const videoResponse = await fetch('http://localhost:5173/api/generate-remotion-video', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        images: imageUrls,
    ...
    if (videoResult.success && videoResult.videoUrl) {
      ...
      // Save as Nova demo
      const outputPath = path.join(__dirname, '..', 'public', 'demo-videos', 'beverly-hills-nova.mp4');
      fs.writeFileSync(outputPath, buffer);
      
      console.log(`✅ Nova demo created: ${(buffer.length / 1024 / 1024).toFixed(2)} MB`);
      console.log(`📁 Saved: /demo-videos/beverly-hills-nova.mp4`);
      console.log('🎯 Now Nova voice selection will play video WITH Nova audio!');
...
async function main() {
  ...
  console.log('   This will replace the current Nova video with one that has audio');
  ...
  if (success) {
    ...
    console.log('🎬 Test: Select Nova voice in demo and video will play WITH audio');
