...
async function generateAllDemoVideos() {
  ...
  for (const voice of VOICES) {
    ...
    try {
      ...
      const ttsResponse = await fetch(
        "http://localhost:5174/api/generate-tts",
        {
          method: "POST",
          headers: {
      ...
      const videoResponse = await fetch(
        "http://localhost:5174/api/generate-remotion-video",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (videoResult.success && videoResult.videoUrl) {
        ...
        const outputPath = path.join(
          demoVideosDir,
          `beverly-hills-${voice.id}.mp4`,
        );
        fs.writeFileSync(outputPath, buffer);

        console.log(
          `  ✅ ${voice.name} video saved: ${(buffer.length / 1024 / 1024).toFixed(2)} MB`,
        );
      ...
      console.error(
        `  ❌ Error generating ${voice.name} video:`,
        error.message,
      );
      results.push({
        voice: voice.id,
  ...
  if (metadata.successCount > 0) {
    ...
    results
      ...
      .forEach((r) => {
        console.log(
          `  - beverly-hills-${r.voice}.mp4 (${(r.size / 1024 / 1024).toFixed(2)} MB)`,
        );
      });
  }
}
