...
async function testOpenAITTS() {
  ...
  try {
    ...
    console.log('Estimated cost: $' + openaiTTSService.estimateCost(propertyScript).toFixed(4));
    console.log('\nGenerating speech...');
    ...
    const echoAudio = await openaiTTSService.generateSpeech(shortScript, { voice: 'echo' });
    ...
    if (error.message.includes('API key')) {
      ...
      console.log('\n⏳ Rate Limit:');
      console.log('You may have hit the OpenAI API rate limit. Please try again later.');
    } else {
      console.log('\n🐛 Debug info:');
      console.log(error);
    }
