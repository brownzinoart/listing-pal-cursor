...
async function runTest(scenario) {
  ...
  console.log(`${colors.cyan}Platform: ${scenario.payload.platform || 'youtube'}${colors.reset}`);
  console.log(`${colors.cyan}Images: ${scenario.payload.images.length}${colors.reset}`);
  console.log(`${colors.cyan}Audio: ${scenario.payload.audioUrl ? 'Yes' : 'No'}${colors.reset}`);
  ...
  try {
    ...
    const duration = ((Date.now() - startTime) / 1000).toFixed(2);
    console.log(`${colors.red}${colors.bright}❌ Request failed!${colors.reset}`);
...
async function runAllTests() {
  console.log(`${colors.bright}${colors.cyan}🚀 Remotion Video Generation Test Suite${colors.reset}`);
  console.log(`${colors.cyan}API URL: ${API_URL}${colors.reset}`);
  console.log(`${colors.cyan}Total scenarios: ${scenarios.length}${colors.reset}`);
  ...
  try {
    ...
    console.log(`${colors.yellow}Please start the Express server with: npm run server${colors.reset}`);
    process.exit(1);
  }
...
if (args.includes('--scenario')) {
  ...
  if (scenario) {
    ...
    console.log(`${colors.red}Scenario not found: ${scenarioName}${colors.reset}`);
