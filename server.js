...
const DEMO_DECOR8_CACHE = {
  ...
  modern_livingroom:
    "https://prod-files.decor8.ai/customer-images/decor8ai_api_user_13493/0d23deb0-4304-4c9d-aea7-1931ce1f5db4.jpg",
  contemporary_bedroom:
    "https://prod-files.decor8.ai/customer-images/decor8ai_api_user_13493/ccdafb28-cbc0-47ed-9d12-72d53e594ef7.jpg",
  scandinavian_kitchen:
    "https://images.unsplash.com/photo-1556909114-4f6447b45a12?w=800&h=600&fit=crop&auto=format",
  industrial_bathroom:
...
const getOllamaService = async () => {
  try {
    const { OllamaService: OllamaServiceClass } = await import(
...
class Decor8AIService {
  ...
  async generateDesign(imageUrl, options = {}) {
    ...
    try {
      ...
      if (options.isAsync) {
        console.log(
          "✅ Async request detected. Forwarding response to client poller.",
        );
      ...
      if (
        ...
        ["processing", "pending"].includes(response.data.status)
      ) {
        console.log(
          "⏳ Decor8AI job requires polling. Starting server-side polling...",
        );
      ...
      if (["failed", "error"].includes(response.data.status)) {
        throw new Error(
          `Decor8AI job failed: ${response.data.error || "Unknown error"}`,
        );
      }

      console.error(
        "❌ Unexpected Decor8AI response structure:",
        response.data,
      );
      throw new Error(
        "Interior redesign service returned an unexpected response format.",
      );
    } catch (error) {
  ...
  async pollForCompletion(
    ...
    console.log(
      `🔄 Starting to poll for job ${jobId} (max ${maxAttempts} attempts, ${intervalMs}ms interval)`,
    );
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        console.log(
          `🔍 Poll attempt ${attempt}/${maxAttempts} for job ${jobId}:`,
          statusResponse.data.status,
        );

        if (
          statusResponse.data.status === "completed" &&
          statusResponse.data.info?.images?.length > 0
        ) {
          console.log("✅ Decor8AI job completed successfully!");
          console.log(
            "🖼️ Generated image URL:",
            statusResponse.data.info.images[0].url,
          );
        ...
        if (
          ...
          console.log(
            `⏳ Job ${jobId} still ${statusResponse.data.status}, continuing to poll...`,
          );
          continue;
        }
        ...
        console.warn(
          `⚠️ Job ${jobId} has unknown status: ${statusResponse.data.status}`,
        );
      } catch (pollError) {
        console.warn(
          `⚠️ Poll attempt ${attempt} failed for job ${jobId}:`,
          pollError.message,
        );
        ...
        if (
          ...
          console.log(
            "🔄 Network timeout, continuing to poll with longer delay...",
          );
        ...
        if (pollError.response?.status >= 500 && attempt < maxAttempts - 2) {
          console.log(
            "🔄 Server error, continuing to poll (server might be temporarily unavailable)...",
          );
          continue;
        }
        ...
        if (attempt === maxAttempts) {
          throw new Error(
            `Interior redesign polling failed after ${maxAttempts} attempts: ${pollError.message}`,
          );
        }
    ...
    throw new Error(
      `Interior redesign job did not complete within ${(maxAttempts * intervalMs) / 1000} seconds. The job may still be processing - please try again later.`,
    );
  }
...
app.post("/api/redesign", upload.single("image"), async (req, res) => {
  try {
    ...
    try {
      ...
      console.warn(
        "⚠️ Decor8AI failed, using demo fallback:",
        decor8Error.message,
      );
      ...
      const fallbackUrl =
        DEMO_DECOR8_CACHE[cacheKey] || DEMO_DECOR8_CACHE["modern_livingroom"];
...
app.post("/api/redesign-url", async (req, res) => {
  ...
  try {
    ...
    if (imageUrl.startsWith("data:image/")) {
      ...
      const uploadResult = await decor8ai.uploadImageToCloudinary(
        imageBuffer,
        `room-upload-${Date.now()}.jpg`,
      );
    ...
    try {
      ...
      if (result.error) {
        ...
        throw new Error(
          `The design service reported an error: ${result.error}`,
        );
      }
      ...
      if (["failed", "error"].includes(result.status)) {
        const errorMessage =
          result.error || "Job failed without a specific error message.";
      ...
      console.error(
        "❌ Unexpected response from Decor8AI service. It does not contain a job_id, image, or a known error structure:",
        result,
      );
      throw new Error(
        "Received an unexpected response from the design service.",
      );
    } catch (decor8Error) {
      console.warn(
        "⚠️ Decor8AI failed, using demo fallback:",
    ...
    res.status(500).json({
      success: false,
      error: error.message || "Failed to generate room redesign",
    });
  }
});
...
app.get("/api/redesign-status/:jobId", async (req, res) => {
  ...
  try {
    ...
    if (error.message.includes("did not complete")) {
      return res
        .status(202)
        .json({ status: "processing", message: "Job is still processing." });
    }
    res.status(500).json({
      status: "failed",
...
app.post("/api/test-ollama-ads", async (req, res) => {
  try {
    ...
    try {
      ...
      if (ollamaService) {
        adCopy = await ollamaService.generateAdCopy(
          listing,
          platform || "linkedin",
...
const generateContentWithFallback = async (
  ...
  if (GeminiService) {
    try {
      ...
      switch (contentType) {
        case "description":
          content = await GeminiService.generatePropertyDescription(
            propertyData,
            style,
          );
          break;
        case "social":
          ...
          break;
        case "email":
          content = await GeminiService.generateEmailContent(
            propertyData,
            style,
          );
          break;
        case "flyer":
          content = await GeminiService.generateFlyerContent(
            propertyData,
            style,
          );
          break;
  ...
  try {
    ...
    if (ollamaService) {
      ...
      switch (contentType) {
        case "description":
          content = await ollamaService.generatePropertyDescription(
            propertyData,
            style,
          );
          break;
        case "social":
          if (platform === "facebook") {
            ...
            content =
              await ollamaService.generateInstagramCaption(propertyData);
...
app.post("/api/listings/generate-social", async (req, res) => {
  try {
    ...
    const result = await generateContentWithFallback("social", propertyData, {
      platform,
      style,
    });
...
app.post("/api/listings/generate-email", async (req, res) => {
  try {
    ...
    const result = await generateContentWithFallback("email", propertyData, {
      style,
    });
...
app.post("/api/listings/generate-flyer", async (req, res) => {
  try {
    ...
    const result = await generateContentWithFallback("flyer", propertyData, {
      style,
    });
...
app.get("/api/crime/fbi/*", async (req, res) => {
  try {
    ...
    const FBI_API_KEY =
      process.env.FBI_API_KEY || "mWhmAmBRTq8kspJNw6UvEhUzPHnOAnlQuygN8lnc";
...
app.get("/api/walkscore", async (req, res) => {
  try {
    ...
    const WS_API_KEY =
      process.env.WS_API_KEY || "5139d659545c4f1a58d0c003fa2f1cb0";
...
app.get("/api/geoapify/places", async (req, res) => {
  try {
    ...
    const GEOAPIFY_API_KEY =
      process.env.GEOAPIFY_API_KEY || "11c892c511b94bbea562a1a768df5af9";
...
app.get("/api/census/*", async (req, res) => {
  try {
    ...
    const CENSUS_API_KEY =
      process.env.CENSUS_API_KEY || "39777ef1581c0b65f8dd55868da60cfe7c1036d1";
...
app.get("/api/attom/*", async (req, res) => {
  try {
    ...
    const ATTOM_API_KEY =
      process.env.ATTOM_API_KEY || "229dcb5876a25c6617aed42098f902af";

    // Construct URL with query parameters
    const url = new URL(
      `https://api.gateway.attomdata.com/propertyapi/v1.0.0${path}`,
    );
...
app.post("/api/property", async (req, res) => {
  ...
  // ================================================================
  console.log(
    "⚠️ ATTOM lookup failed or no ATTOM_API_KEY, returning empty property details",
  );
  return res.status(404).json({
...
async function getPropertyDataForAutoFill(address) {
    ...
    try {
      ...
      if (
        ...
        if (p?.building?.size?.livingSize || p?.building?.size?.universalSize) {
          data.squareFootage =
            p.building.size.livingSize || p.building.size.universalSize;
  ...
  if (missingAfterAttom.length > 0) {
    try {
      console.log(
        "🏠 Attempting RentCast lookup for missing fields:",
        missingAfterAttom,
      );
      const rentcastHeaders = { "X-Api-Key": process.env.RENTCAST_API_KEY };
      const propertyUrl = `https://api.rentcast.io/v1/properties?address=${encodeURIComponent(address)}`;
...
app.post("/api/generate-remotion-video", async (req, res) => {
  try {
    ...
    const { remotionVideoService } = await import(
      "./services/remotionVideoService.js"
    );
...
app.post("/api/generate-tts", async (req, res) => {
  try {
    ...
    if (!text) {
      return res
        .status(400)
        .json({ error: "Text is required for TTS generation" });
    }
...
app.listen(PORT, () => {
  ...
  console.log(
    "🔗 Vite frontend should be configured to proxy API requests to this port.",
  );
});
...
[Your Contact Information]`,
  ...
  if (type === "property-description") {
    content =
      fallbackContent["property-description"][
        req.query.style || "professional"
      ];
...
app.post("/api/listings/generate-paid-ads", async (req, res) => {
  try {
    ...
    console.log(
      "🎯 DEMO: Generating instant paid ads content for presentation",
    );
