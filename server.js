...
class Decor8AIService {
  ...
  async generateDesign(imageUrl, options = {}) {
    ...
    try {
      ...
      if (
        ...
        response.data.info.images.length > 0
      ) {
  ...
  async pollForCompletion(
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        if (
          ...
          throw new Error(
            `Decor8AI job failed: ${statusResponse.data.error || "Unknown error"}`,
          );
        }
        ...
        if (
          ...
          console.log(
            `⏳ Job still ${statusResponse.data.status}, continuing to poll...`,
          );
          continue;
        }
      } catch (pollError) {
        ...
        if (attempt === maxAttempts) {
          throw new Error(
            `Polling failed after ${maxAttempts} attempts: ${pollError.message}`,
          );
        }
      }
    }
    ...
    throw new Error(
      `Decor8AI job did not complete within ${(maxAttempts * intervalMs) / 1000} seconds`,
    );
  }
...
app.post("/api/redesign-url", async (req, res) => {
  try {
    ...
    console.log(
      "🔍 Final Decor8AI result structure:",
    ...
    if (result.info && result.info.images && result.info.images.length > 0) {
      ...
      throw new Error(
        "No images received from Decor8AI - check API response structure",
      );
    }
  } catch (error) {
