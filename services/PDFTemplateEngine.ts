...
export class PDFTemplateEngine {
  ...
  private drawGlassPanel(x: number, y: number, width: number, height: number, opacity: number = 0.95): void {
    ...
    this.pdf.roundedRect(x, y, width, height, PDF_LAYOUT.elements.borderRadius, PDF_LAYOUT.elements.borderRadius, 'F');
    
    // Subtle border
    this.pdf.setDrawColor(200, 200, 220);
    this.pdf.setLineWidth(0.3);
    this.pdf.roundedRect(x, y, width, height, PDF_LAYOUT.elements.borderRadius, PDF_LAYOUT.elements.borderRadius, 'S');
  }

  private drawBrandGradient(x: number, y: number, width: number, height: number): void {
  ...
  private drawShadow(x: number, y: number, width: number, height: number): void {
    // Subtle shadow effect using light gray instead of alpha
    const offset = PDF_LAYOUT.elements.shadowOffset;
    this.pdf.setFillColor(230, 230, 230);
    this.pdf.roundedRect(x + offset, y + offset, width, height, PDF_LAYOUT.elements.borderRadius, PDF_LAYOUT.elements.borderRadius, 'F');
  }

  // Page templates
  async generateCoverPage(listing: Listing, agent: AgentProfile): Promise<void> {
    ...
    await this.addHeroPropertyImage(listing, PDF_LAYOUT.page.margin, this.currentY, 180, imageHeight);
    ...
    this.pdf.roundedRect(PDF_LAYOUT.page.margin - 5, this.currentY - 12, 100, 30, 5, 5, 'F');
  ...
  generateOverviewPage(listing: Listing): void {
    ...
    if (listing.generatedDescription) {
      ...
      this.pdf.roundedRect(PDF_LAYOUT.page.margin, this.currentY, 170, 60, 5, 5, 'F');
      
      this.setFont(PDF_TYPOGRAPHY.sizes.h3, 'bold');
      this.setColor(PDF_COLORS.textPrimary);
      this.pdf.text('Executive Summary', PDF_LAYOUT.page.margin + 5, this.currentY + 10);
      
      const summaryText = listing.generatedDescription.substring(0, 250) + '...';
  ...
  generateDescriptionPage(listing: Listing): void {
    ...
    if (listing.generatedDescription) {
      ...
      this.pdf.text('Property description will be available soon.', PDF_LAYOUT.page.margin, this.currentY);
    }
  ...
  generateSocialMediaPage(listing: Listing): void {
    ...
    if (listing.generatedFacebookPost) {
      this.addSocialMediaPost('Facebook', listing.generatedFacebookPost, PDF_COLORS.facebook);
      this.currentY += 15;
    }
    
    // Instagram Post
    if (listing.generatedInstagramCaption) {
      this.addSocialMediaPost('Instagram', listing.generatedInstagramCaption, PDF_COLORS.instagram);
      this.currentY += 15;
    }
    
    // X/Twitter Post
    if (listing.generatedXPost) {
      this.addSocialMediaPost('X (Twitter)', listing.generatedXPost, PDF_COLORS.twitter);
    }
  ...
  generateEmailCampaignPage(listing: Listing): void {
    ...
    this.pdf.roundedRect(PDF_LAYOUT.page.margin, this.currentY, 170, 200, 5, 5, 'F');
    ...
    emailLines.forEach(line => {
      if (emailY < this.currentY + 190) { // Stay within container
  ...
  async generateRoomRedesignsPage(listing: Listing): Promise<void> {
    ...
    designsToShow.forEach((design, index) => {
      // Design container
      this.pdf.setFillColor(248, 249, 250);
      this.pdf.roundedRect(PDF_LAYOUT.page.margin, this.currentY, 170, 80, 5, 5, 'F');
      
      // Style name
      this.setFont(PDF_TYPOGRAPHY.sizes.h3, 'bold');
      this.setColor(PDF_COLORS.textPrimary);
      this.pdf.text(design.styleId || `Design ${index + 1}`, PDF_LAYOUT.page.margin + 5, this.currentY + 10);
      ...
      if (design.prompt) {
        ...
        lines.slice(0, 3).forEach((line, i) => {
          this.pdf.text(line, PDF_LAYOUT.page.margin + 5, this.currentY + 20 + (i * 6));
        });
      }
  ...
  async generateMarketingMaterialsPage(listing: Listing): Promise<void> {
    if (!listing.generatedFlyers || listing.generatedFlyers.length === 0) return;
    ...
    this.setFont(PDF_TYPOGRAPHY.sizes.body);
    this.setColor(PDF_COLORS.textSecondary);
    this.pdf.text(`${listing.generatedFlyers.length} professional marketing flyers have been created for this property.`, PDF_LAYOUT.page.margin, this.currentY);
    this.currentY += 15;
    ...
    listing.generatedFlyers.slice(0, 5).forEach((flyer, index) => {
      // Flyer item
      this.pdf.setFillColor(248, 249, 250);
      this.pdf.roundedRect(PDF_LAYOUT.page.margin, this.currentY, 170, 25, 3, 3, 'F');
      
      this.setFont(PDF_TYPOGRAPHY.sizes.body, 'bold');
      this.setColor(PDF_COLORS.textPrimary);
      this.pdf.text(`Flyer ${index + 1}`, PDF_LAYOUT.page.margin + 5, this.currentY + 8);
      ...
      if (flyer.customization) {
        ...
        colors.forEach((color, i) => {
          ...
          this.pdf.circle(PDF_LAYOUT.page.margin + 100 + (i * 15), this.currentY + 8, 3, 'F');
        });
      }
  ...
  generateNeighborhoodInsightsPage(listing: Listing): void {
    ...
    const hasCustomSections = listing.neighborhoodSections && listing.neighborhoodSections.length > 0;
    ...
    // Neighborhood name
    const neighborhood = listing.address ? listing.address.split(',')[0] : 'Local Area';
    this.setFont(PDF_TYPOGRAPHY.sizes.h3, 'bold');
    this.setColor(PDF_COLORS.primary);
    this.pdf.text(`${neighborhood} Neighborhood`, PDF_LAYOUT.page.margin, this.currentY);
    this.currentY += 15;
    ...
    if (hasCustomSections && listing.neighborhoodSections) {
      listing.neighborhoodSections.forEach((section, index) => {
        ...
        // Section container
        this.pdf.setFillColor(248, 249, 250);
        this.pdf.roundedRect(PDF_LAYOUT.page.margin, this.currentY, 170, 40, 5, 5, 'F');
        ...
        if (content) {
          ...
          contentLines.slice(0, 3).forEach((line, i) => {
            this.pdf.text(line, PDF_LAYOUT.page.margin + 5, this.currentY + 20 + (i * 5));
          });
        }
      ...
      this.setFont(PDF_TYPOGRAPHY.sizes.body);
      this.setColor(PDF_COLORS.textSecondary);
      this.pdf.text('Neighborhood insights are available in the online listing portal.', PDF_LAYOUT.page.margin, this.currentY);
      this.currentY += 10;
      this.pdf.text('Visit the listing page to explore:', PDF_LAYOUT.page.margin, this.currentY);
      this.currentY += 8;
    ...
    if (this.currentY < 200) {
      ...
      this.setFont(PDF_TYPOGRAPHY.sizes.small);
      this.setColor(PDF_COLORS.textTertiary);
      this.pdf.text('Interactive neighborhood map available online', PDF_LAYOUT.page.margin + 40, this.currentY + 30);
    }
  ...
  generatePaidAdsPage(listing: Listing): void {
    ...
    try {
      ...
      this.setFont(PDF_TYPOGRAPHY.sizes.body);
      this.setColor(PDF_COLORS.textPrimary);
      const adText = typeof listing.generatedAdCopy === 'string' 
        ? listing.generatedAdCopy 
    ...
    adCampaigns.slice(0, 3).forEach(ad => {
      // Ad container
      this.pdf.setFillColor(248, 249, 250);
      this.pdf.roundedRect(PDF_LAYOUT.page.margin, this.currentY, 170, 50, 5, 5, 'F');
      
      // Platform and objective
      this.setFont(PDF_TYPOGRAPHY.sizes.h4, 'bold');
      this.setColor(PDF_COLORS.primary);
      this.pdf.text(`${ad.platform} - ${ad.objective}`, PDF_LAYOUT.page.margin + 5, this.currentY + 10);
      
      // Headline
      this.setFont(PDF_TYPOGRAPHY.sizes.body, 'bold');
      this.setColor(PDF_COLORS.textPrimary);
      this.pdf.text(ad.headline || '', PDF_LAYOUT.page.margin + 5, this.currentY + 20);
      ...
      bodyLines.slice(0, 2).forEach((line, i) => {
        this.pdf.text(line, PDF_LAYOUT.page.margin + 5, this.currentY + 28 + (i * 5));
      });
      
      // CTA
      this.setFont(PDF_TYPOGRAPHY.sizes.small, 'bold');
      this.setColor(PDF_COLORS.accent);
      this.pdf.text(ad.cta || '', PDF_LAYOUT.page.margin + 5, this.currentY + 42);
  ...
  private addCleanFooter(): void {
    ...
    this.pdf.setDrawColor(226, 232, 240);
    this.pdf.setLineWidth(0.5);
    this.pdf.line(PDF_LAYOUT.page.margin, footerY - 5, PDF_LAYOUT.page.width - PDF_LAYOUT.page.margin, footerY - 5);
  ...
  private addContentSummaryPage(contentSummary: any): void {
    ...
    this.setFont(PDF_TYPOGRAPHY.sizes.h3, 'bold');
    this.setColor(PDF_COLORS.textPrimary);
    this.pdf.text('PDF Content Included:', PDF_LAYOUT.page.margin, this.currentY);
    this.currentY += 15;
    
    // Content checklist
    const contentItems = [
      { label: 'Property Overview', included: true },
      { label: 'Property Description', included: contentSummary.hasDescription },
      { label: 'Photo Gallery', included: contentSummary.hasImages && contentSummary.imagesLength > 0 },
      { label: 'Social Media Posts', included: contentSummary.hasFacebookPost || contentSummary.hasInstagramCaption || contentSummary.hasXPost },
      { label: 'Email Campaign', included: contentSummary.hasEmail },
      { label: 'Interior Redesigns', included: contentSummary.hasRoomDesigns },
      { label: 'Marketing Flyers', included: contentSummary.hasFlyers && contentSummary.flyersLength > 0 },
      { label: 'Digital Ad Campaigns', included: contentSummary.hasAdCopy },
      { label: 'Neighborhood Insights', included: contentSummary.hasNeighborhoodSections || contentSummary.hasCoordinates }
    ];
    
    contentItems.forEach(item => {
      // Checkbox
      this.setFont(PDF_TYPOGRAPHY.sizes.body);
      this.setColor(item.included ? PDF_COLORS.primary : PDF_COLORS.textTertiary);
      this.pdf.text(item.included ? '✓' : '○', PDF_LAYOUT.page.margin, this.currentY);
      
      // Label
      this.setColor(item.included ? PDF_COLORS.textPrimary : PDF_COLORS.textTertiary);
    ...
    if (contentSummary.roomDesignsType === 'string' || contentSummary.adCopyType === 'string') {
      this.currentY += 15;
      this.setFont(PDF_TYPOGRAPHY.sizes.small);
      this.setColor(PDF_COLORS.textTertiary);
      this.pdf.text('Note: Some content required format conversion during PDF generation.', PDF_LAYOUT.page.margin, this.currentY);
    }
  ...
  private async addCleanAgentHeader(agent: AgentProfile): Promise<void> {
    ...
    this.pdf.circle(PDF_LAYOUT.page.margin + photoSize/2, this.currentY + photoSize/2, photoSize/2, 'F');
    ...
    if (agent.photoUrl) {
      try {
        ...
        this.pdf.addImage(photoData, 'JPEG', PDF_LAYOUT.page.margin, this.currentY, photoSize, photoSize);
      } catch (error) {
  ...
  private addModernStatsCards(listing: Listing): void {
    const stats = [
      ...
      { icon: '📐', label: 'Square Feet', value: listing.squareFootage.toLocaleString() },
    ...
    stats.forEach((stat, index) => {
      ...
      this.setFont(PDF_TYPOGRAPHY.sizes.h3, 'bold');
      this.setColor(PDF_COLORS.textPrimary);
      this.pdf.text(stat.value, x + cardWidth/2 - (stat.value.length * 2), this.currentY + 16);
      
      // Label
      this.setFont(PDF_TYPOGRAPHY.sizes.caption);
      this.setColor(PDF_COLORS.textTertiary);
      this.pdf.text(stat.label, x + cardWidth/2 - (stat.label.length * 1.5), this.currentY + 23);
    });
  ...
  private addDetailedPropertyInfo(listing: Listing): void {
    ...
    const details = [
      ...
      { label: 'Price per Sq Ft', value: `$${Math.round(listing.price / listing.squareFootage)}` },
      { label: 'Monthly Est.', value: `$${Math.round(listing.price * 0.004).toLocaleString()}` }
    ];
    ...
    features.slice(0, 8).forEach(feature => {
      if (feature.trim()) {
        ...
        lines.forEach((line, i) => {
          this.pdf.text(line, PDF_LAYOUT.page.margin + 8, this.currentY + (i * 6));
        });
  ...
  private addSocialMediaPost(platform: string, content: string, brandColor: string): void {
    // Social media post container
    const containerHeight = 50;
    this.pdf.setFillColor(248, 249, 250);
    this.pdf.roundedRect(PDF_LAYOUT.page.margin, this.currentY, 170, containerHeight, 5, 5, 'F');
    
    // Platform indicator
    const rgb = this.hexToRgb(brandColor);
    this.pdf.setFillColor(rgb.r, rgb.g, rgb.b);
    this.pdf.rect(PDF_LAYOUT.page.margin, this.currentY, 3, containerHeight, 'F');
    ...
    lines.slice(0, 5).forEach((line, i) => {
      this.pdf.text(line, PDF_LAYOUT.page.margin + 8, this.currentY + 20 + (i * 5));
    });
  }
  ...
  private async addPremiumAgentHeader(agent: AgentProfile): Promise<void> {
    ...
    this.pdf.setLineWidth(0.5);
    this.pdf.line(PDF_LAYOUT.page.margin, headerHeight - 8, PDF_LAYOUT.page.width - PDF_LAYOUT.page.margin, headerHeight - 8);
  }

  private async addPremiumAgentPhoto(agent: AgentProfile, x: number, y: number, size: number): Promise<void> {
  ...
  private drawPremiumAgentPhotoPlaceholder(x: number, y: number, size: number): void {
  ...
  private addPremiumStatsBar(listing: Listing): void {
    ...
    this.drawGlassPanel(PDF_LAYOUT.page.margin - 5, barY - 5, 180, barHeight, 0.9);
    
    const stats = [
      { label: 'Beds', value: listing.bedrooms.toString(), icon: '🛏️' },
      { label: 'Baths', value: listing.bathrooms.toString(), icon: '🚿' },
      { label: 'Sq Ft', value: listing.squareFootage.toLocaleString(), icon: '📐' },
  ...
  private addPremiumFooter(): void {
    ...
    this.setFont(PDF_TYPOGRAPHY.sizes.caption);
    this.setColor(PDF_COLORS.textTertiary);
    this.pdf.text(`${this.pageNumber}`, PDF_LAYOUT.page.width - PDF_LAYOUT.page.margin - 5, footerY);
    
    // ListingPal branding with style
    this.setFont(PDF_TYPOGRAPHY.sizes.caption, 'bold');
    this.setColor(PDF_COLORS.primary);
    this.pdf.text('Powered by ListingPal', PDF_LAYOUT.page.width - PDF_LAYOUT.page.margin - 45, footerY);
  }

  // Component methods
  private async addAgentPhoto(agent: AgentProfile, x: number, y: number, size: number): Promise<void> {
    if (agent.photoUrl) {
      try {
        ...
        console.warn('⚠️ Could not load agent photo, using placeholder:', error);
  ...
  private async addAgentHeader(agent: AgentProfile): Promise<void> {
    ...
    this.pdf.line(PDF_LAYOUT.page.margin, headerHeight - 5, PDF_LAYOUT.page.width - PDF_LAYOUT.page.margin, headerHeight - 5);
  }

  private async addHeroPropertyImage(listing: Listing, x: number, y: number, width: number, height: number): Promise<void> {
    ...
    if (listing.images && listing.images.length > 0) {
      try {
        ...
        console.warn('⚠️ Could not load hero image, using premium placeholder:', error);
  ...
  private drawPremiumPropertyImagePlaceholder(x: number, y: number, width: number, height: number): void {
    // Shadow effect
    this.drawShadow(x, y, width, height);
    ...
    this.setFont(PDF_TYPOGRAPHY.sizes.body, 'bold');
    this.setColor(PDF_COLORS.printBackground);
    this.pdf.text('Premium Property Image', x + width/2 - 35, y + height/2 + 5);
    
    this.setFont(PDF_TYPOGRAPHY.sizes.caption);
    this.setColor(PDF_COLORS.textTertiary);
    this.pdf.text('Professional photography showcase', x + width/2 - 40, y + height/2 + 15);
  }

  private drawPropertyImagePlaceholder(x: number, y: number, width: number, height: number): void {
  ...
  private addPropertyStatsGrid(listing: Listing): void {
    const stats = [
      ...
      { label: 'Square Footage', value: listing.squareFootage.toLocaleString() + ' sq ft', icon: '📐' },
      { label: 'Year Built', value: listing.yearBuilt.toString(), icon: '🏗️' },
      { label: 'Property Type', value: listing.propertyType || 'Single Family', icon: '🏠' },
      { label: 'Listing Type', value: listing.listingType === 'sale' ? 'For Sale' : 'For Rent', icon: '💰' }
    ];
  ...
  private addFormattedText(text: string, fontSize: number): void {
    ...
    const lines = this.pdf.splitTextToSize(text, PDF_LAYOUT.page.width - (PDF_LAYOUT.page.margin * 2));
  ...
  private addPageFooter(): void {
    ...
    // ListingPal branding
    this.pdf.text('Powered by ListingPal', PDF_LAYOUT.page.width - PDF_LAYOUT.page.margin - 40, footerY);
  }

  // Utility methods
  private parseKeyFeatures(keyFeatures: string): string[] {
    if (keyFeatures.includes('•') || keyFeatures.includes('·')) {
      return keyFeatures.split(/[•·]/g)
        .map(feat => feat.trim())
  ...
  async generatePDF(listing: Listing, agent: AgentProfile): Promise<void> {
    ...
    const contentSummary = {
      ...
      roomDesignsLength: Array.isArray(listing.generatedRoomDesigns) ? listing.generatedRoomDesigns.length : 'not array',
    ...
    try {
      ...
      try {
        ...
        throw new Error(`Failed to generate cover page: ${coverError instanceof Error ? coverError.message : 'Unknown error'}`);
      }
      ...
      try {
        ...
        throw new Error(`Failed to generate overview page: ${overviewError instanceof Error ? overviewError.message : 'Unknown error'}`);
      }
      ...
      if (listing.generatedFacebookPost || listing.generatedInstagramCaption || listing.generatedXPost) {
      ...
      if (listing.generatedRoomDesigns) {
        ...
        if (roomDesigns && roomDesigns.length > 0) {
          console.log(`🏠 Generating room redesigns page with ${roomDesigns.length} designs...`);
          try {
            await this.generateRoomRedesignsPage(listing);
          } catch (roomError) {
            console.error('⚠️ Room redesigns page generation failed:', roomError);
          }
        } else {
      ...
      if (listing.generatedFlyers && listing.generatedFlyers.length > 0) {
        ...
        try {
          ...
          console.error('⚠️ Marketing materials page generation failed:', flyerError);
        }
      }
      ...
      if ((listing.neighborhoodSections && listing.neighborhoodSections.length > 0) || 
        ...
        try {
          ...
          console.error('⚠️ Neighborhood insights page generation failed:', neighborhoodError);
        }
      }
