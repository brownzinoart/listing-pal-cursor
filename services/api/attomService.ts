...
export class AttomService {
  ...
  async getPropertyDetails(lat: number, lng: number): Promise<AttomPropertyData | null> {
    try {
      const params = {
  ...
  async getMarketData(zipCode: string): Promise<AttomMarketData> {
    try {
      ...
      try {
        const response = await this.makeRequest('/property/snapshot', { postalcode: zipCode, pagesize: '10' });
  ...
  async getComparables(lat: number, lng: number, radius: number = 0.5): Promise<AttomComparables> {
    try {
      const params = {
        latitude: lat.toString(),
        longitude: lng.toString(),
      ...
      const squareFootages = properties
        .filter((p: any) => p.building && p.building.size && p.building.size.universalsize)
        .map((p: any) => parseInt(p.building.size.universalsize));
      
      const averagePrice = prices.length > 0 ? 
        prices.reduce((sum: number, price: number) => sum + price, 0) / prices.length : 0;
      
      const averageSquareFootage = squareFootages.length > 0 ? 
        squareFootages.reduce((sum: number, sqft: number) => sum + sqft, 0) / squareFootages.length : 0;
  ...
  async getPropertyValuation(lat: number, lng: number): Promise<{
  ...
  private determineMarketTrend(monthsOfSupply: number): 'buyer' | 'seller' | 'balanced' {
