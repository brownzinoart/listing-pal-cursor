...
export class CensusService {
  ...
  async getDemographicsByState(stateCode: string, year: number = 2022): Promise<CensusData> {
    try {
  ...
  async getDemographicsByCounty(stateCode: string, countyCode: string, year: number = 2022): Promise<CensusData> {
    try {
      const variables = [
        'B01003_001E', // Total population
        'B25003_001E', // Total housing units
  ...
  private parseCensusResponse(data: any[], year: number): CensusData {
    ...
    const totalEducationPop = data.length > 8 ? safeNumber(data[8]) : totalPopulation;
    ...
    const laborForce = data.length > 12 ? safeNumber(data[12]) : totalPopulation * 0.6;

    // Calculate derived metrics
    const unemploymentRate = laborForce > 0 ? (unemployed / laborForce) * 100 : 0;
    const collegeEducatedPercent = totalEducationPop > 0 ? (collegeGrads / totalEducationPop) * 100 : 0;
    const ownerOccupiedPercent = totalHousingUnits > 0 ? (ownerOccupied / totalHousingUnits) * 100 : 0;
