...
export class FBICrimeService {
  ...
  async getCrimeDataByState(stateAbbr: string, year: number = 2022): Promise<CrimeData> {
    try {
  ...
  async getCrimeDataByAgency(ori: string, year: number = 2022): Promise<AgencyCrimeData | null> {
    try {
  ...
  private calculateTrend(crimeRate: number): 'improving' | 'stable' | 'worsening' {
