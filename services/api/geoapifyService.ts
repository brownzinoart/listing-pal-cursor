...
export class GeoapifyService {
  ...
  async getNearbyPlaces(lat: number, lng: number, categories: string[], radius: number = 2000): Promise<GeoapifyPlace[]> {
  ...
  async getRestaurants(lat: number, lng: number): Promise<GeoapifyPlace[]> {
    return this.getNearbyPlaces(lat, lng, ['catering.restaurant', 'catering.fast_food', 'catering.cafe']);
  }

  async getShopping(lat: number, lng: number): Promise<GeoapifyPlace[]> {
    return this.getNearbyPlaces(lat, lng, ['commercial.supermarket', 'commercial.shopping_mall', 'commercial.convenience']);
  }

  async getEntertainment(lat: number, lng: number): Promise<GeoapifyPlace[]> {
    return this.getNearbyPlaces(lat, lng, ['entertainment.cinema', 'entertainment.museum', 'leisure.park', 'entertainment.theme_park']);
  }

  async getEssentialServices(lat: number, lng: number): Promise<GeoapifyPlace[]> {
    return this.getNearbyPlaces(lat, lng, ['service', 'commercial.health_and_beauty.pharmacy', 'healthcare']);
  }

  async getSchools(lat: number, lng: number): Promise<GeoapifyPlace[]> {
    return this.getNearbyPlaces(lat, lng, ['education.school', 'education.college', 'education.university', 'childcare.kindergarten']);
  }
  ...
  async getSafetyServices(lat: number, lng: number): Promise<GeoapifyPlace[]> {
    return this.getNearbyPlaces(lat, lng, ['emergency', 'healthcare.hospital', 'service']);
  }

  async getAllAmenities(lat: number, lng: number) {
    try {
      const [restaurants, shopping, entertainment, services, schools, transportation, safety] = await Promise.all([
      ...
      return {
        ...
        transportation,
        safety,
        totalCount: restaurants.length + shopping.length + entertainment.length + services.length
      };
    } catch (error) {
