    ...
    try {
      ...
      if (initialData.success && initialData.imageUrl) {
        ...
        try {
          ...
          console.log(
            "✅ Decor8AI image URL format is valid and ready for use",
          );
        } catch (urlError) {
        ...
        console.log(
          "🎉 Decor8AI processing completed successfully - proceeding to next step",
        );
      ...
      if (initialData.jobId) {
        console.log(
          `⏳ Received job ID: ${initialData.jobId}. Starting to poll for Decor8AI results...`,
        );
        ...
        const pollUntilComplete = async (
          ...
          for (let i = 0; i < retries; i++) {
            try {
              ...
              console.log(
                `🔄 Polling attempt ${i + 1}/${retries} for Decor8AI job ${initialData.jobId}`,
              );

              const statusResponse = await fetch(
                `/api/redesign-status/${initialData.jobId}`,
                {
                  method: "GET",
                  headers: { "Content-Type": "application/json" },
                },
              ...
              if (statusResponse.ok) {
                ...
                console.log(
                  `📊 Decor8AI job ${initialData.jobId} status:`,
                  statusData.status,
                );

                if (statusData.status === "completed" && statusData.imageUrl) {
                  console.log(
                    `✅ Decor8AI job ${initialData.jobId} completed successfully!`,
                  );
                ...
                console.log(
                  `⏳ Decor8AI job ${initialData.jobId} still ${statusData.status}, continuing to poll...`,
                );
              } else if (statusResponse.status === 202) {
                // Job is still processing (202 Accepted)
                console.log(
                  `⏳ Decor8AI job ${initialData.jobId} still processing (202), continuing to poll...`,
                );
              } else {
                // Other error status
