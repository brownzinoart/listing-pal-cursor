  ...
  async generateActualRoomRedesign(
    ...
    try {
      ...
      if (initialData.success && initialData.imageUrl) {
        ...
        console.log(
          "🔄 Starting polling to ensure image is fully processed...",
        );

        const pollUntilImageReady = async (
          imageUrl: string,
          retries = 15,
          delay = 3000,
        ): Promise<string> => {
          for (let i = 0; i < retries; i++) {
            try {
              console.log(
                `🔄 Polling attempt ${i + 1}/${retries} to verify image is ready: ${imageUrl.substring(0, 50)}...`,
              );
              ...
              if (imgResponse.ok) {
                ...
                console.log(
                  "✅ Image is fully processed and accessible:",
                  imageUrl,
                );
                return imageUrl;
              } else {
                console.log(
                  `⏳ Image not ready yet (HTTP ${imgResponse.status}), waiting ${delay}ms...`,
                );
              }
            } catch (error) {
              console.log(
                `⏳ Image verification attempt ${i + 1} failed, waiting ${delay}ms...`,
          ...
          throw new Error(
            `Image processing verification timed out after ${retries} attempts. The image may not be fully ready.`,
          );
        };

        const verifiedImageUrl = await pollUntilImageReady(
