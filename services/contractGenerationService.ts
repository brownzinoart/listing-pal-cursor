...
export class ContractGenerationService {
    ...
    sections: [
      ...
      },
      {
        ...
        editableFields: [
          ...
          "financials.earnestMoneyDeposit",
          "financials.earnestMoneyHolder",
        ],
      },
      {
        id: "dates",
      ...
      },
      {
        ...
        required: true,
        content:
          "During the Due Diligence Period, Buyer shall have the right to terminate this Contract for any reason or no reason by providing written notice to Seller prior to the expiration of the Due Diligence Period. If Buyer terminates during Due Diligence, the Earnest Money Deposit shall be refunded to Buyer.",
        editableFields: [],
      },
    ],
    standardClauses: [
      {
        ...
        category: "financial",
        content:
          "Seller shall pay for deed preparation, excise tax (revenue stamps), and their attorney fees. Buyer shall pay for deed recording, their loan costs, and their attorney fees.",
        isStandard: true,
        stateSpecific: ["NC"],
      },
      {
        ...
        category: "legal",
        content:
          "North Carolina law requires that an attorney licensed in North Carolina must oversee the closing of all real estate transactions.",
        isStandard: true,
        stateSpecific: ["NC"],
      },
      {
        ...
        category: "property",
        content:
          "Seller makes no representation as to the ownership of any mineral and oil/gas rights. Any mineral and oil/gas rights owned by Seller, if any, shall be conveyed to Buyer at Closing unless otherwise stated herein.",
        isStandard: true,
        stateSpecific: ["NC"],
      },
    ],
  ...
  private static getTemplateByStateAndType(
    state: string,
    contractType: string,
  ): ContractTemplate {
  ...
  static formatContractForDisplay(
    contract: Contract,
...
${
  contract.listingAgent
    ? `**Listing Agent:** ${contract.listingAgent.firstName} ${contract.listingAgent.lastName} (${contract.listingAgent.brokerage})  
License #: ${contract.listingAgent.licenseNumber}`
    : "N/A"
}

${
  contract.sellingAgent
    ? `**Selling Agent:** ${contract.sellingAgent.firstName} ${contract.sellingAgent.lastName} (${contract.sellingAgent.brokerage})  
License #: ${contract.sellingAgent.licenseNumber}`
    : "N/A"
}
...
${
  contract.buyerAttorney
    ? `**Buyer's Attorney:** ${contract.buyerAttorney.firstName} ${contract.buyerAttorney.lastName}  
Firm: ${contract.buyerAttorney.firm}  
Bar #: ${contract.buyerAttorney.barNumber}`
    : `**Buyer's Attorney:** To be determined`
}

${
  contract.sellerAttorney
    ? `**Seller's Attorney:** ${contract.sellerAttorney.firstName} ${contract.sellerAttorney.lastName}  
Firm: ${contract.sellerAttorney.firm}  
Bar #: ${contract.sellerAttorney.barNumber}`
    : `**Seller's Attorney:** To be determined`
}
...
`;
  ...
  static validateContract(contract: Partial<Contract>): {
    isValid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
  ...
  static calculateDates(
    ...
    const dates: ContractDates = {
      ...
      offerExpirationDate: new Date(
        offerDate.getTime() + 2 * 24 * 60 * 60 * 1000,
      ).toISOString(), // 2 days
      dueDiligenceDeadline: new Date(
        offerDate.getTime() + 21 * 24 * 60 * 60 * 1000,
      ).toISOString(), // 21 days for NC
      closingDate: new Date(
        offerDate.getTime() + closingDays * 24 * 60 * 60 * 1000,
      ).toISOString(),
      ...
      inspectionDeadline: new Date(
        offerDate.getTime() + 10 * 24 * 60 * 60 * 1000,
      ).toISOString(),
