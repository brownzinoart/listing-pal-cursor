...
export class ContractPDFService {
  ...
  static async generateContractPDF(
    ...
    try {
      ...
      pdf.text("Standard Form 2-T", this.PAGE_WIDTH / 2, yPosition, {
        align: "center",
      });
      yPosition += 20;
      ...
      pdf.text(
        `Property Address: ${contract.property?.address || listing.address}`,
        this.MARGIN,
        yPosition,
      );
      yPosition += 7;
      pdf.text(
        `City: ${contract.property?.city || listing.city}, State: ${contract.property?.state || "NC"}, ZIP: ${contract.property?.zipCode || listing.zipCode}`,
        this.MARGIN,
        yPosition,
      );
      yPosition += 7;
      ...
      if (contract.property?.legalDescription) {
        pdf.text(
          `Legal Description: ${contract.property.legalDescription}`,
          this.MARGIN,
          yPosition,
        );
        yPosition += 7;
      ...
      contract.buyers?.forEach((buyer, index) => {
        ...
        if (buyer.address) {
          pdf.text(
            `   ${buyer.address}, ${buyer.city}, ${buyer.state} ${buyer.zipCode}`,
            this.MARGIN + 5,
            yPosition,
          );
          yPosition += 5;
        ...
        if (buyer.email || buyer.phone) {
          pdf.text(
            `   Email: ${buyer.email || "N/A"} | Phone: ${buyer.phone || "N/A"}`,
            this.MARGIN + 5,
            yPosition,
          );
          yPosition += 5;
      ...
      contract.sellers?.forEach((seller, index) => {
        pdf.text(
          `${index + 1}. ${seller.firstName} ${seller.lastName}`,
          this.MARGIN + 5,
          yPosition,
        );
        yPosition += 5;
        ...
        if (seller.email || seller.phone) {
          pdf.text(
            `   Email: ${seller.email || "N/A"} | Phone: ${seller.phone || "N/A"}`,
            this.MARGIN + 5,
            yPosition,
          );
          yPosition += 5;
      ...
      pdf.text(
        `Purchase Price: $${this.formatCurrency(contract.financials?.purchasePrice || 0)}`,
        this.MARGIN,
        yPosition,
      );
      yPosition += 7;
      pdf.text(
        `Earnest Money Deposit: $${this.formatCurrency(contract.financials?.earnestMoneyDeposit || 0)}`,
        this.MARGIN,
        yPosition,
      );
      yPosition += 7;
      pdf.text(
        `Financing Type: ${this.capitalizeFirst(contract.financials?.financingType || "Not specified")}`,
        this.MARGIN,
        yPosition,
      );
      yPosition += 7;
      ...
      if (contract.financials?.downPaymentAmount) {
        pdf.text(
          `Down Payment: $${this.formatCurrency(contract.financials.downPaymentAmount)} (${contract.financials.downPaymentPercentage?.toFixed(1)}%)`,
          this.MARGIN,
          yPosition,
        );
        yPosition += 7;
      ...
      if (contract.financials?.loanAmount) {
        pdf.text(
          `Loan Amount: $${this.formatCurrency(contract.financials.loanAmount)}`,
          this.MARGIN,
          yPosition,
        );
        yPosition += 7;
      ...
      pdf.text(
        `Offer Date: ${this.formatDate(contract.dates?.offerDate)}`,
        this.MARGIN,
        yPosition,
      );
      yPosition += 7;
      pdf.text(
        `Offer Expiration: ${this.formatDate(contract.dates?.offerExpirationDate)}`,
        this.MARGIN,
        yPosition,
      );
      yPosition += 7;
      pdf.text(
        `Due Diligence Deadline: ${this.formatDate(contract.dates?.dueDiligenceDeadline)} at 5:00 PM`,
        this.MARGIN,
        yPosition,
      );
      yPosition += 7;
      pdf.text(
        `Closing Date: ${this.formatDate(contract.dates?.closingDate)}`,
        this.MARGIN,
        yPosition,
      );
      yPosition += 7;
      pdf.text(
        `Possession Date: ${this.formatDate(contract.dates?.possessionDate)} at ${contract.dates?.possessionTime || "6:00 PM"}`,
        this.MARGIN,
        yPosition,
      );
      yPosition += 10;
      ...
      pdf.text(
        `Property Condition: ${this.capitalizeFirst(contract.property?.propertyCondition?.replace("-", " ") || "Not specified")}`,
        this.MARGIN,
        yPosition,
      );
      yPosition += 10;
      ...
      if (
        contract.property?.includedItems &&
        contract.property.includedItems.length > 0
      ) {
        pdf.setFont("helvetica", "bold");
      ...
      if (
        contract.property?.excludedItems &&
        contract.property.excludedItems.length > 0
      ) {
        pdf.setFont("helvetica", "bold");
      ...
      if (
        ...
        contract.contingencies.filter((c) => c.included).length > 0
      ) {
        // Check if we need a new page
        if (yPosition > this.PAGE_HEIGHT - 80) {
          pdf.addPage();
        ...
        contract.contingencies
          ...
          .forEach((contingency) => {
            pdf.setFont("helvetica", "bold");
            pdf.text(
              `${this.capitalizeFirst(contingency.type)} Contingency:`,
              this.MARGIN,
              yPosition,
            );
            yPosition += 5;
            ...
            if (contingency.deadline) {
              pdf.text(
                `Deadline: ${this.formatDate(contingency.deadline)}`,
                this.MARGIN + 5,
                yPosition,
              );
              yPosition += 5;
      ...
      if (
        contract.specialStipulations &&
        contract.specialStipulations.length > 0
      ) {
        // Check if we need a new page
      ...
      pdf.text(
        `Lead Paint Disclosure: ${contract.property?.leadPaintDisclosure ? "Provided" : "Not Required/Not Provided"}`,
        this.MARGIN,
        yPosition,
      );
      yPosition += 7;
      pdf.text(
        `Property Disclosure Statement: ${contract.property?.propertyDisclosureProvided ? "Provided" : "Not Provided"}`,
        this.MARGIN,
        yPosition,
      );
      yPosition += 7;
      pdf.text(
        `Mineral Rights: ${contract.property?.mineralRightsIncluded ? "Included" : "Not Included"}`,
        this.MARGIN,
        yPosition,
      );
      yPosition += 15;
      ...
      if (contract.buyerAttorney) {
        ...
        pdf.text(
          `Name: ${contract.buyerAttorney.firstName} ${contract.buyerAttorney.lastName}`,
          this.MARGIN,
          yPosition,
        );
        yPosition += 7;
        ...
        if (contract.buyerAttorney.phone) {
          pdf.text(
            `Phone: ${contract.buyerAttorney.phone}`,
            this.MARGIN,
            yPosition,
          );
          yPosition += 7;
      ...
      for (let i = 1; i <= pageCount; i++) {
        ...
        pdf.setFontSize(8);
        pdf.text(
          `NC Contract - Page ${i} of ${pageCount}`,
          this.PAGE_WIDTH / 2,
          this.PAGE_HEIGHT - 10,
          { align: "center" },
        );
        pdf.text(
          "Generated by ListingPal",
  ...
  private static splitTextToLines(
    pdf: jsPDF,
    text: string,
    maxWidth: number,
  ): string[] {
