...
export const DEMO_PROPERTIES: DemoProperty[] = [
  {
    ...
    description: 'Nestled in the prestigious Benedict Canyon, this architectural masterpiece redefines luxury living. Designed by renowned architect Richard Landry, this estate offers unparalleled sophistication with breathtaking views from downtown to the Pacific.'
  },
  {
    ...
    description: 'This stunning Mediterranean Modern estate combines old-world elegance with contemporary luxury. Situated on a private cul-de-sac above Sunset Strip, enjoy explosive city views and ultimate privacy in this meticulously crafted home.'
  },
  {
    ...
    description: 'Experience the epitome of California coastal living in this stunning beachfront estate. With 100 feet of pristine beach frontage and unobstructed ocean views, this architectural marvel offers the perfect blend of luxury and natural beauty.'
  }
];
...
const FALLBACK_IMAGES = {
  ...
  living_room: 'https://images.unsplash.com/photo-1600210492486-724fe5c67fb0?w=1200&q=80',
  ...
  bathroom: 'https://images.unsplash.com/photo-1620626011761-996317b8d101?w=1200&q=80',
      ...
      try {
        ...
        if (data.results && data.results.length > 0) {
          ...
          images.push({
            ...
            description: photo.description || photo.alt_description || `${category.replace('_', ' ')} view`,
            category,
 ...
 */
export async function convertImagesToFiles(images: PropertyImage[]): Promise<File[]> {
  const files: File[] = [];
...
export async function getDemoPropertyWithImages(propertyId?: string): Promise<{
  ...
  const propertyType = property.address.includes('Beach') ? 'beach' : 
                      property.price > 10000000 ? 'mansion' : 'estate';
...
export function generatePropertyScript(property: DemoProperty): {
  ...
  const location = property.address.includes('Beach') ? 'beachfront' : 
                  property.address.includes('Beverly Hills') ? 'Beverly Hills' : 
                  'West Hollywood';

  return {
    intro: `Welcome to this exceptional ${property.style || 'luxury'} estate located at ${property.address}. ${property.description}`,
    features: property.features.slice(0, 5).map(feature => 
    ...
    }).format(property.price)}, this ${location} masterpiece represents a rare opportunity to own one of the area's most prestigious properties. Contact us today to schedule your private showing.`
  };
}