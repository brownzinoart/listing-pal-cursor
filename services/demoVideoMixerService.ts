...
export class DemoVideoMixerService {
  ...
  async mixVideoWithVoice(
    ...
    try {
      ...
      resourceManagementService.registerResource("blob_url", voiceBlobUrl, {
        type: "audio",
        voice: voiceId,
        size: voiceBlob.size,
      });
      ...
      const baseVideoFile = new File([baseVideoBlob], "base-video.mp4", {
        type: "video/mp4",
      });

      // Use slideshow service's merge functionality
      const mergedVideoUrl =
        await slideshowVideoService.mergeAudioVideoFromBlobs(
          baseVideoFile,
          voiceBlobUrl,
        );
   ...
   */
  async preCachePopularVoices(
    voiceIds: string[] = ["nova", "echo"],
  ): Promise<void> {
  ...
  getCacheStats() {
    return {
      ...
      voices: Array.from(this.mixedVideoCache.keys()).map((key) =>
        key.split("-").pop(),
      ),
