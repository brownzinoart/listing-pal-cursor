...
export class LocationContextService {
  ...
  async getAllLocationContext(address: string): Promise<LocationContextData> {
    try {
      ...
      const [
        ...
        this.walkScoreAPI.getWalkScore(
          coordinates.lat,
          coordinates.lng,
          address,
        ),
        this.censusAPI.getDemographics(coordinates.lat, coordinates.lng),
        this.googlePlacesAPI.getAllNearbyAmenities(
          coordinates.lat,
          coordinates.lng,
        ),
  ...
  private categorizeCards(
    cards: ContextCard[],
  ): LocationContextData["categorizedCards"] {
    return {
      ...
      amenities: cards.filter((c) =>
        ["restaurants", "shopping", "parks", "recreation"].includes(c.id),
      ),
