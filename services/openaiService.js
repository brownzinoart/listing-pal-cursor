    ...
    try {
      ...
      const completion = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an expert real estate copywriter with extensive experience in creating compelling property descriptions that convert leads into buyers. You understand market psychology and can adapt your writing style to different audiences and property types.'
          },
          {
            role: 'user',
            content: prompt
...
Your goal is maximum accuracy by intelligently combining data from the specified real estate portals.`;
      ...
      const completion = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a knowledgeable real estate data analyst who provides realistic, conservative estimates and avoids making up specific details that could be inaccurate. You focus on providing helpful contextual information and reasonable estimates.'
          },
          {
            role: 'user',
...
Generate the property description now:`;
  ...
  buildSocialMediaPrompt(propertyData, platform, style) {
    ...
    const platformSpecs = {
      ...
      instagram: { charLimit: 2200, hashtags: 10, tone: 'visual and lifestyle-focused' },
      twitter: { charLimit: 280, hashtags: 2, tone: 'concise and punchy' },
      linkedin: { charLimit: 1300, hashtags: 3, tone: 'professional and business-focused' }
    };
    ...
    try {
      ...
      const completion = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a real estate marketing specialist who creates compelling flyer content that drives inquiries and showings.'
          },
          {
            role: 'user',
            content: prompt
    ...
    try {
      ...
      const completion = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an email marketing expert specializing in real estate communications that generate leads and drive engagement.'
          },
          {
            role: 'user',
            content: prompt
...
Important: Provide realistic, conservative estimates based on general knowledge of the area. If specific data isn't available, provide general guidance and suggest verification of details. Focus on helpful contextual information that would be valuable to potential property buyers or investors.`;
      ...
      const completion = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a knowledgeable real estate neighborhood analyst who provides realistic insights based on general market knowledge. You avoid making up specific statistics and focus on providing helpful contextual information.'
          },
          {
            role: 'user',
            content: prompt
      ...
      try {
        ...
        console.log('Could not parse neighborhood insights JSON, using raw text');
        return {
    ...
    try {
      const completion = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'user',
            content: 'Test message - please respond with "OpenAI connection successful"'
          }
        ],
        max_tokens: 10,
      });
