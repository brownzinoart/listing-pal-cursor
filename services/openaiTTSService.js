...
export class OpenAITTSService {
  constructor(apiKey) {
    ...
    if (!key) {
      console.warn(
        "⚠️ OpenAI API key not found. TTS features will be disabled. Set OPENAI_API_KEY or VITE_OPENAI_API_KEY to enable.",
      );
  ...
  async generateSpeech(text, options = {}) {
    if (!this.isAvailable) {
      ...
      throw new Error(
        "OpenAI TTS Service not available. Please configure API key.",
      );
    }
    try {
