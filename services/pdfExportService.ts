...
export class PDFExportService {
    ...
    try {
      ...
      if (elementId) {
        ...
        try {
          ...
          console.error('⚠️ Element capture failed, falling back to programmatic PDF:', captureError);
      ...
      if (imgHeight > this.PAGE_HEIGHT - yPosition - this.MARGIN) {
        ...
        for (let i = 0; i < totalPages; i++) {
          ...
          if (tempCtx) {
            ...
            pdf.addImage(tempImgData, 'PNG', this.MARGIN, yPosition, imgWidth, tempImgHeight);
  ...
  private static async generateProgrammaticPDF(
    ...
    pdf.text(listing.address.split(',').slice(1).join(',').trim(), this.MARGIN, yPosition);
    yPosition += 15;
  ...
  private static async addAgentHeader(pdf: jsPDF, agent: AgentProfile): Promise<void> {
  ...
  private static parseKeyFeatures(keyFeatures: string): string[] {
    ...
    if (keyFeatures.includes('•') || keyFeatures.includes('·') || keyFeatures.includes('*')) {
