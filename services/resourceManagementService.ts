...
class ResourceManagementService {
  ...
  registerResource(
    ...
    console.log(
      `📦 Resource registered: ${type} (${id}) - ${this.formatBytes(size || 0)}`,
    );
      ...
      switch (resource.type) {
        case "blob_url":
          if (
            ...
            resource.resource.startsWith("blob:")
          ) {
        ...
        case "audio_context":
          if (
            resource.resource &&
            typeof resource.resource.close === "function"
          ) {
        ...
        case "ffmpeg_instance":
          if (
            resource.resource &&
            typeof resource.resource.cleanup === "function"
          ) {
  ...
  private checkMemoryPressure(): void {
    ...
    if (stats.memoryPressure === "high") {
      console.warn(
        "🚨 High memory pressure detected, cleaning up old resources",
      );
  ...
  getResourceStats(): ResourceStats {
    ...
    const memoryPressure: "low" | "medium" | "high" =
      totalMemoryUsage > this.memoryThreshold
        ? "high"
        : totalMemoryUsage > this.memoryWarningThreshold
          ? "medium"
          : "low";
      ...
      setInterval(() => {
        ...
        if (memory) {
          ...
          if (usedMB > limitMB * 0.8) {
            console.warn(
              `🧠 High JS heap usage: ${usedMB.toFixed(1)}MB / ${limitMB.toFixed(1)}MB`,
            );
            this.cleanupOldResources();
          }
        }
  ...
  getMemoryRecommendations(): string[] {
    ...
    if (stats.memoryPressure === "high") {
      recommendations.push(
        "Consider reducing the number of images or video quality",
      );
    ...
    if (stats.totalResources > 50) {
      recommendations.push(
        "Too many resources active - consider restarting the workflow",
      );
    }
