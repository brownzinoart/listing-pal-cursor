...
const HUGGINGFACE_API_KEY = import.meta.env.VITE_HUGGINGFACE_API_KEY || process.env.HUGGINGFACE_API_KEY;
...
class VideoGenerationService {
  ...
  private demoProperty = {
    ...
    rooms: ['Grand Living Room', 'Gourmet Kitchen', 'Master Suite', 'Resort-Style Backyard', 'Spa Bathroom'],
    features: ['Marble Floors', 'Custom Cabinetry', 'Walk-in Closet', 'Infinity Pool', 'Rain Shower'],
    script: "Welcome to 123 Luxury Lane, where modern luxury meets timeless elegance in the heart of Beverly Hills."
  };
  ...
  async analyzeImages(images: File[]): Promise<VideoAnalysis> {
    try {
      ...
      if (images.length > 20) {
        throw new Error('Too many images. Maximum 20 images allowed for processing.');
      }
      ...
      if (totalSize > maxSize) {
        throw new Error('Images are too large. Please reduce file sizes or image count.');
      }
      ...
      return {
        detectedRooms: this.demoProperty.rooms.slice(0, Math.min(images.length, 5)),
        keyFeatures: this.demoProperty.features.slice(0, Math.min(images.length, 5)),
        imageQuality: {
      ...
      if (error instanceof Error) {
        ...
        if (error.message.includes('Network') || error.message.includes('connection')) {
          throw new Error('Image analysis failed due to network issues. Please check your connection and try again.');
        }
      }
      
      // Generic fallback error
      throw new Error('Image analysis failed. Please try again or contact support if the problem persists.');
    }
  }

  // Generate AI script with enhanced demo content
  async generateScript(listing: Listing, analysis: VideoAnalysis, style: string = 'modern'): Promise<VideoScript> {
    try {
      // Validate inputs
      if (!listing) {
        throw new Error('Property listing data is required for script generation');
      }

      if (!analysis || !analysis.detectedRooms || analysis.detectedRooms.length === 0) {
        throw new Error('Image analysis data is required for script generation');
      }
      ...
      const propertyFeatures = (listing as any).features || this.demoProperty.features;
    ...
    const template = styleTemplates[style as keyof typeof styleTemplates] || styleTemplates.modern;

    // Enhanced scene generation using property features
    const scenes = analysis.detectedRooms.map((room, index) => ({
      imageIndex: index,
      narration: this.generateEnhancedSceneNarration(room, analysis.keyFeatures[index], propertyFeatures, style),
      ...
      return {
        ...
        totalDuration: scenes.reduce((sum, scene) => sum + scene.duration, 0) + 15 // +15 for intro/outro
      };
    } catch (error) {
      ...
      if (error instanceof Error) {
        ...
        if (error.message.includes('service temporarily unavailable')) {
          throw new Error('Script generation service is temporarily unavailable. Please try again in a few moments.');
        }
      }
      
      // Generic fallback error
      throw new Error('Failed to generate video script. Please try again or contact support if the problem persists.');
    }
  }

  // Generate voice preview using OpenAI TTS
  async generateVoicePreview(text: string, voiceId: string): Promise<string | null> {
    try {
  ...
  async generateVoiceover(
    ...
    try {
      return await ttsWithFallback.generateSpeech(script, undefined, onProviderChange);
    } catch (error) {
  ...
  private async generateVoiceoverWithOpenAI(script: VideoScript): Promise<string | null> {
  ...
  private async generateVoiceoverWithElevenLabs(script: VideoScript): Promise<string | null> {
    ...
    try {
      // Combine all script parts into full narration
      const fullScript = `${script.intro} ${script.scenes.map(s => s.narration).join(' ')} ${script.outro}`;
      console.log('Sending script to ElevenLabs:', fullScript.substring(0, 100) + '...');
      ...
      resourceManagementService.registerResource(
        'blob_url',
        audioUrl,
        { type: 'audio', service: 'elevenlabs', size: audioBlob.size }
      );
  ...
  async generateVideoWithUrls(
    ...
    propertyInfo?: { address: string; price: string; beds: number; baths: number; sqft: number }
    ...
    try {
      ...
      let audioUrl = await this.generateVoiceover(script, (providerId, providerName) => {
  ...
  async generateVideo(
    ...
    try {
      ...
      if (totalImageSize > maxSize) {
        throw new Error('Images are too large for video processing. Please reduce file sizes.');
      }
      ...
      return {
        ...
        platformVersions: {
          ...
          youtube: { url: videoResult.videoUrl, duration: script.totalDuration }
        }
      };
    } catch (error) {
      console.error('Video generation error:', error);
      ...
      if (error instanceof Error) {
        ...
        if (error.message.includes('memory') || error.message.includes('heap')) {
          onProgress?.(100, "Video generation failed due to insufficient memory");
          throw new Error('Insufficient memory to generate video. Please try with fewer or smaller images.');
        }
        ...
        if (error.message.includes('network') || error.message.includes('fetch') || error.message.includes('API')) {
          console.log('Network error encountered, attempting fallback generation...');
          ...
          try {
            ...
            const fallbackVideoUrl = await slideshowVideoService.createSlideshow(
              images,
            ...
            return {
              ...
              platformVersions: {
                ...
                youtube: { url: fallbackVideoUrl, duration: script.totalDuration }
              }
            };
          } catch (fallbackError) {
            console.error('Fallback video generation also failed:', fallbackError);
            onProgress?.(100, "Video generation failed");
            throw new Error('Video generation failed. All backup methods exhausted. Please try again later.');
          }
        }
      }
      ...
      return {
        ...
        platformVersions: {
          ...
          instagram: { url: `/api/videos/fallback/instagram.mp4`, duration: 90 },
          youtube: { url: `/api/videos/fallback/youtube.mp4`, duration: script.totalDuration }
        }
      };
    }
  }
  ...
  async optimizeForPlatforms(
    ...
    for (const platform of platforms) {
      const messages = {
        tiktok: ["Selecting best moments...", "Adjusting to 9:16 ratio...", "Adding trending music..."],
        instagram: ["Optimizing for Reels...", "Adjusting pacing...", "Enhancing colors..."],
        youtube: ["Preparing full tour...", "Adding chapters...", "Optimizing quality..."]
      };
  ...
  private generateEnhancedSceneNarration(room: string, feature: string, propertyFeatures: string[], style: string): string {
    ...
    } else if (roomType.includes('exterior') || roomType.includes('pool') || roomType.includes('outdoor')) {
    ...
    const selectedFeature = specificFeatures[Math.floor(Math.random() * specificFeatures.length)];
    ...
    const templates = styleTemplates[style as keyof typeof styleTemplates] || styleTemplates.modern;
    return templates[Math.floor(Math.random() * templates.length)];
  }
