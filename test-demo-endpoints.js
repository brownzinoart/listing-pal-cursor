...
const testData = {
  propertyData: {
    ...
    keyFeatures: "Modern kitchen, hardwood floors, updated bathrooms, private backyard"
  }
};
...
async function testEndpoint(name, endpoint, data) {
  try {
    ...
    console.log(`   Content length: ${response.data.description?.length || response.data.content?.length || 'N/A'} chars`);
...
async function checkServer() {
  try {
    ...
    console.log(`   Ollama: ${response.data.ollamaStatus === 'available' ? '✅' : '❌'}`);
    return true;
  } catch (error) {
    console.log('❌ Server is not running. Please start with: npm run dev:full');
    return false;
  }
}
